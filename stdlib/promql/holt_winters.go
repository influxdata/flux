...
func init() {
	...
	flux.RegisterPackageValue("promql", HoltWintersKind, flux.FunctionValue(HoltWintersKind, createHoltWintersOpSpec, holtWintersSignature))
...
func (t *holtWintersTransformation) Process(id execute.DatasetID, tbl flux.Table) error {
	...
	if err := builder.AppendFloat(valIdx, s1); err != nil {
		return err
	}
