...
testcase add_durations {
        got =
            csv.from(csv: inData)
                ...
                |> map(fn: (r) => ({r with _value: date.minute(t: date.add(d: -1m, to: r._time))}))
            ...
            csv.from(csv: inData)
                ...
                |> map(
                    fn: (r) =>
                        ({r with _value:
                                date.minute(
                                    t: date.add(d: -1m, to: r._time, location: {zone: "Asia/Kolkata", offset: 0h}),
                                ),
                        }),
                )
...
testcase sub_durations {
        got =
            csv.from(csv: inData)
                |> range(start: 2018-01-01T00:00:00Z)
                |> map(fn: (r) => ({r with _value: date.minute(t: date.sub(d: -1m, from: r._time))}))
            ...
            csv.from(csv: inData)
                ...
                |> map(
                    fn: (r) =>
                        ({r with _value:
                                date.minute(
                                    t: date.sub(d: -1m, from: r._time, location: {zone: "Asia/Kolkata", offset: 0h}),
                                ),
                        }),
                )
