...
testcase week_day_duration_location {
    got = csv.from(csv: inData)
        |> range(start: 2018-01-01T00:00:00Z)
        |> map(fn: (r) => ({r with _value: date.weekDay(t: duration(v: r._value), location: {zone: "America/Los_Angeles", offset: 0m})}))
