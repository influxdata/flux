...
testcase array_concatstr {
    input =
        array.from(
            ...
            |> range(start: 2018-05-22T19:53:00Z, stop: 2018-05-22T19:54:00Z)
    want =
        array.from(rows: [{_measurement: "diskio", sarr: "[15204688,15204894,15205102]"}])
            |> group(columns: ["_measurement"])
    ...
    got =
        input
            ...
            |> reduce(
                fn: (r, accumulator) => ({sarr: barray.concatStr(arr: accumulator.sarr, v: [float(v: r._value)])}),
                identity: {sarr: barray.emptyStr},
            )
