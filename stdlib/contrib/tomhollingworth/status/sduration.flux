...
//
// ### Similar functions
// `status.sduration()` is similar to `elapsed()` and `stateDuration()`, but differs in important ways:
//
// - `elapsed()` drops the first record. `status.sduration()` does not.
// - `stateDuration()` calculates the total time spent in a state (determined by a predicate function).
//   `status.sduration()` returns the duration between all records and their subsequent records.
//
...
// `status.sduration()`, `elapsed()`, and `stateDuration()`
...
// union(
//     tables: [
//         data |> status.sduration(unit: 1m, stop: 2020-01-02T00:00:00Z) |> map(fn: (r) => ({_time: r._time, state: r.state, function: "status.sduration()", value: r.duration})),
