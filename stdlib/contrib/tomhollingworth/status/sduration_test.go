...
import (
	...
	"github.com/influxdata/flux/stdlib/contrib/tomhollingworth/status"
	...
	tests := []querytest.NewQueryTestCase{
		{
			...
			Raw:     `import "contrib/tomhollingworth/status" from(bucket:"mydb") |> range(start:-1h) |> drop(columns: ["_stop"]  |> status.sduration()`,
			WantErr: true,
		},
		{
			Name:    "duration missing time column",
			Raw:     `import "contrib/tomhollingworth/status" from(bucket:"mydb") |> range(start:-1h) |> drop(columns: ["_time"]  |> status.sduration()`,
			WantErr: true,
		},
		{
			Name:    "duration default",
			Raw:     `import "contrib/tomhollingworth/status" from(bucket:"mydb") |> range(start:-1h)  |> status.sduration()`,
			WantErr: false,
			Want: &flux.Spec{
		...
		},
		{
			...
			Raw:     `import "contrib/tomhollingworth/status" from(bucket:"mydb") |> range(start:-1h)  |> status.sduration(unit: 1ms, timeColumn: "start", stopColumn: "end", columnName: "result")`,
			WantErr: false,
			Want: &flux.Spec{
				Operations: []*flux.Operation{
					{
		...
		},
		{
			...
			Raw:     `import "contrib/tomhollingworth/status" from(bucket:"mydb") |> range(start:-1h)  |> status.sduration(stop: 2020-10-20T08:30:00Z)`,
			WantErr: false,
			Want: &flux.Spec{
				Operations: []*flux.Operation{
					{
...
func TestDuration_PassThrough(t *testing.T) {
	executetest.TransformationPassThroughTestHelper(t, func(d execute.Dataset, c execute.TableBuilderCache) execute.Transformation {
		s := status.NewDurationTransformation(
			d,
			c,
			&status.sdurationProcedureSpec{},
		)
		return s
	})
}
...
func TestDuration_DurationProcedureSpec(t *testing.T) {
	...
	s := status.sdurationProcedureSpec{
...
func TestDuration_Process(t *testing.T) {
	testCases := []struct {
		name string
		spec *status.sdurationProcedureSpec
		data []flux.Table
		...
		t.Run(tc.name, func(t *testing.T) {
			executetest.ProcessTestHelper(
				...
				func(d execute.Dataset, c execute.TableBuilderCache) execute.Transformation {
					return status.NewDurationTransformation(d, c, tc.spec)
				},
			)
		})
	}
