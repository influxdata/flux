...
import (
	...
	"github.com/mvn-trinhnguyen2-dn/flux/stdlib/contrib/trinhnguyen/status"
	...
	tests := []querytest.NewQueryTestCase{
		{
			...
			Raw:     `import "contrib/trinhnguyen/status" from(bucket:"mydb") |> range(start:-1h) |> drop(columns: ["_stop"]  |> status.statusDuration()`,
			WantErr: true,
		},
		{
			Name:    "duration missing time column",
			Raw:     `import "contrib/trinhnguyen/status" from(bucket:"mydb") |> range(start:-1h) |> drop(columns: ["_time"]  |> status.statusDuration()`,
			WantErr: true,
		},
		{
			Name:    "duration default",
			Raw:     `import "contrib/trinhnguyen/status" from(bucket:"mydb") |> range(start:-1h)  |> status.statusDuration()`,
			WantErr: false,
			Want: &flux.Spec{
		...
		},
		{
			...
			Raw:     `import "contrib/trinhnguyen/status" from(bucket:"mydb") |> range(start:-1h)  |> status.duration(unit: 1ms, timeColumn: "start", stopColumn: "end", columnName: "result")`,
			WantErr: false,
			Want: &flux.Spec{
				Operations: []*flux.Operation{
					{
		...
		},
		{
			...
			Raw:     `import "contrib/trinhnguyen/status" from(bucket:"mydb") |> range(start:-1h)  |> status.duration(stop: 2020-10-20T08:30:00Z)`,
			WantErr: false,
			Want: &flux.Spec{
				Operations: []*flux.Operation{
					{
...
func TestDuration_DurationProcedureSpec(t *testing.T) {
	...
	s := status.DurationProcedureSpec{
...
func TestDuration_Process(t *testing.T) {
	testCases := []struct {
		name string
		spec *status.DurationProcedureSpec
		data []flux.Table
