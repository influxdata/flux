// Package status provides tools for analyzing event-based data.
//
// ## Metadata
// introduced: 0.91.0
package status
...
// duration calculates the duration of status.
//
...
//
// ### Similar functions
// `status.statusDuration()` is similar to `elapsed()` and `stateDuration()`, but differs in important ways:
//
// - `elapsed()` drops the first record. `status.statusDuration()` does not.
// - `stateDuration()` calculates the total time spent in a state (determined by a predicate function).
//   `status.statusDuration()` returns the duration between all records and their subsequent records.
//
...
// `status.statusDuration()`, `elapsed()`, and `stateDuration()`
...
// union(
//     tables: [
//         data |> status.statusDuration(unit: 1m, stop: 2020-01-02T00:00:00Z) |> map(fn: (r) => ({_time: r._time, state: r.state, function: "status.statusDuration()", value: r.duration})),
...
//
// ## Metadata
// tags: transformations,status
//
builtin statusDuration : (
        <-tables: stream[A],
        ?unit: duration,
        ?timeColumn: string,
        ?columnName: string,
