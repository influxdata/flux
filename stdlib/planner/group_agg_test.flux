...
testcase group_one_tag_count {
    want = array.from(rows: [{"t0": "t0v0", "_value": 12}, {"t0": "t0v1", "_value": 12}])
        |> group(columns: ["t0"])
...
testcase group_one_tag_filter_field_count {
    want = array.from(rows: [{"t0": "t0v0", "_value": 6}, {"t0": "t0v1", "_value": 6}])
        |> group(columns: ["t0"])
    got = testing.load(tables: inData)
        |> range(start: -100y)
        |> filter(fn: (r) => r._field == "f0")
...
testcase group_two_tag_filter_field_count {
    want = array.from(rows: [{"t0": "t0v0", "t1": "t1v0", "_value": 3}, {"t0": "t0v0", "t1": "t1v1", "_value": 3}, {"t0": "t0v1", "t1": "t1v0", "_value": 3}, {"t0": "t0v1", "t1": "t1v1", "_value": 3}])
        |> group(columns: ["t0", "t1"])
    got = testing.load(tables: inData)
        |> range(start: -100y)
        |> filter(fn: (r) => r._field == "f0")
...
testcase group_one_tag_sum {
    want = array.from(rows: [{"t0": "t0v0", "_value": 19}, {"t0": "t0v1", "_value": 22}])
        |> group(columns: ["t0"])
    got = testing.load(tables: inData)
        |> range(start: -100y)
        |> group(columns: ["t0"])
...
testcase group_one_tag_filter_field_sum {
    want = array.from(rows: [{"t0": "t0v0", "_value": 12}, {"t0": "t0v1", "_value": 13}])
        |> group(columns: ["t0"])
    got = testing.load(tables: inData)
        |> range(start: -100y)
        |> filter(fn: (r) => r._field == "f0")
...
testcase group_two_tag_filter_field_sum {
    want = array.from(rows: [{"t0": "t0v0", "t1": "t1v0", "_value": 4}, {"t0": "t0v0", "t1": "t1v1", "_value": 8}, {"t0": "t0v1", "t1": "t1v0", "_value": 5}, {"t0": "t0v1", "t1": "t1v1", "_value": 8}])
        |> group(columns: ["t0", "t1"])
    got = testing.load(tables: inData)
        |> range(start: -100y)
        |> filter(fn: (r) => r._field == "f0")
