// DO NOT EDIT: This file is autogenerated via the builtin command.

package geo

import (
	flux "github.com/influxdata/flux"
	ast "github.com/influxdata/flux/ast"
)

func init() {
	flux.RegisterPackage(pkgAST)
}

var pkgAST = &ast.Package{
	BaseNode: ast.BaseNode{
		Errors: nil,
		Loc:    nil,
	},
	Files: []*ast.File{&ast.File{
		BaseNode: ast.BaseNode{
			Errors: nil,
			Loc: &ast.SourceLocation{
				End: ast.Position{
					Column: 30,
					Line:   124,
				},
				File:   "geo.flux",
				Source: "package geo\n\n//\n// None of the builtin functions are intended to be used by end users.\n//\n\n// Check whether lat/lon is in a lat/lon box or center/radius circle.\nbuiltin containsLatLon\n\n// Calculates grid (set of cell ID tokens) for given box and according to options.\nbuiltin getGrid\n\n// Finds parent cell ID token for given cell ID at specified level.\nbuiltin getParent\n\n// Returns level of specified cell ID token.\nbuiltin getLevel\n\n//\n// Flux\n//\n\n// Gets level of cell ID tag `_ci` from the first record from the first table from the stream.\n_detectCiLevel = (tables=<-) => {\n  _r0 =\n    tables\n      |> tableFind(fn: (key) => exists key._ci)\n      |> getRecord(idx: 0)\n  _level =\n    if exists _r0 then\n      getLevel(token: _r0._ci)\n    else\n       666\n  return _level\n}\n\n//\n// Convenience functions\n//\n\n// Collects values to row-wise sets.\ntoRows = (tables=<-, correlationKey=[\"_time\"]) =>\n  tables\n    |> pivot(\n      rowKey: correlationKey,\n      columnKey: [\"_field\"],\n      valueColumn: \"_value\"\n    )\n\n//\n// Filtering functions\n//\n\n// Filters records by a box, a circle or a polygon area using S2 cell ID tag.\n// It is a coarse filter, as the grid always overlays the region, the result will likely contain records\n// with lat/lon outside the specified region.\ngridFilter = (tables=<-, box={}, circle={}, polygon={}, minSize=24, maxSize=-1, level=-1, ciLevel=-1) => {\n  _ciLevel =\n    if ciLevel == -1 then\n      tables\n        |> _detectCiLevel()\n    else\n      ciLevel\n  _grid = getGrid(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, maxLevel: _ciLevel)\n  return\n    tables\n      |> filter(fn: (r) =>\n        if _grid.level == _ciLevel then\n          contains(value: r._ci, set: _grid.set)\n        else\n          contains(value: getParent(token: r._ci, level: _grid.level), set: _grid.set)\n      )\n}\n\n// Filters records by a box, a circle or a polygon region.\n// It is an exact filter and must be used after `toRows()` because it requires `lat` and `lon` columns in input row sets.\nstrictFilter = (tables=<-, box={}, circle={}, polygon={}) =>\n  tables\n    |> filter(fn: (r) =>\n      containsLatLon(box: box, circle: circle, polygon: polygon, lat: r.lat, lon: r.lon)\n    )\n\n// Two-phase filtering by a box, a circle or a polygon region.\n// Returns rows of fields correlated by `correlationKey`.\nfilterRows = (tables=<-, box={}, circle={}, polygon={}, minSize=24, maxSize=-1, level=-1, ciLevel=-1, correlationKey=[\"_time\"], strict=true) => {\n  _rows =\n    tables\n      |> gridFilter(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, ciLevel: ciLevel)\n      |> toRows(correlationKey)\n  _result =\n    if strict then\n      _rows\n        |> strictFilter(box, circle, polygon)\n    else\n      _rows\n  return _result\n}\n\n//\n// Grouping functions\n//\n// intended to be used row-wise sets (i.e after `toRows()`)\n\n// Groups data by area of size specified by level. Result is grouped by `newColumn`.\n// Grouping levels: https://s2geometry.io/resources/s2cell_statistics.html\ngroupByArea = (tables=<-, newColumn, level, ciLevel=-1) => {\n  _prepared =\n    if level == ciLevel then\n      tables\n\t    |> duplicate(column: \"_ci\", as: newColumn)\n    else\n      tables\n        |> map(fn: (r) => ({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) }))\n        |> rename(columns: { _cixxx: newColumn })\n  return _prepared\n    |> group(columns: [newColumn])\n}\n\n// Groups rows into tracks.\nasTracks = (tables=<-, groupBy=[\"id\",\"tid\"], orderBy=[\"_time\"]) =>\n  tables\n    |> group(columns: groupBy)\n    |> sort(columns: orderBy)",
				Start: ast.Position{
					Column: 1,
					Line:   2,
				},
			},
		},
		Body: []ast.Statement{&ast.BuiltinStatement{
			BaseNode: ast.BaseNode{
				Errors: nil,
				Loc: &ast.SourceLocation{
					End: ast.Position{
						Column: 23,
						Line:   9,
					},
					File:   "geo.flux",
					Source: "builtin containsLatLon",
					Start: ast.Position{
						Column: 1,
						Line:   9,
					},
				},
			},
			ID: &ast.Identifier{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 23,
							Line:   9,
						},
						File:   "geo.flux",
						Source: "containsLatLon",
						Start: ast.Position{
							Column: 9,
							Line:   9,
						},
					},
				},
				Name: "containsLatLon",
			},
		}, &ast.BuiltinStatement{
			BaseNode: ast.BaseNode{
				Errors: nil,
				Loc: &ast.SourceLocation{
					End: ast.Position{
						Column: 16,
						Line:   12,
					},
					File:   "geo.flux",
					Source: "builtin getGrid",
					Start: ast.Position{
						Column: 1,
						Line:   12,
					},
				},
			},
			ID: &ast.Identifier{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 16,
							Line:   12,
						},
						File:   "geo.flux",
						Source: "getGrid",
						Start: ast.Position{
							Column: 9,
							Line:   12,
						},
					},
				},
				Name: "getGrid",
			},
		}, &ast.BuiltinStatement{
			BaseNode: ast.BaseNode{
				Errors: nil,
				Loc: &ast.SourceLocation{
					End: ast.Position{
						Column: 18,
						Line:   15,
					},
					File:   "geo.flux",
					Source: "builtin getParent",
					Start: ast.Position{
						Column: 1,
						Line:   15,
					},
				},
			},
			ID: &ast.Identifier{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 18,
							Line:   15,
						},
						File:   "geo.flux",
						Source: "getParent",
						Start: ast.Position{
							Column: 9,
							Line:   15,
						},
					},
				},
				Name: "getParent",
			},
		}, &ast.BuiltinStatement{
			BaseNode: ast.BaseNode{
				Errors: nil,
				Loc: &ast.SourceLocation{
					End: ast.Position{
						Column: 17,
						Line:   18,
					},
					File:   "geo.flux",
					Source: "builtin getLevel",
					Start: ast.Position{
						Column: 1,
						Line:   18,
					},
				},
			},
			ID: &ast.Identifier{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 17,
							Line:   18,
						},
						File:   "geo.flux",
						Source: "getLevel",
						Start: ast.Position{
							Column: 9,
							Line:   18,
						},
					},
				},
				Name: "getLevel",
			},
		}, &ast.VariableAssignment{
			BaseNode: ast.BaseNode{
				Errors: nil,
				Loc: &ast.SourceLocation{
					End: ast.Position{
						Column: 2,
						Line:   36,
					},
					File:   "geo.flux",
					Source: "_detectCiLevel = (tables=<-) => {\n  _r0 =\n    tables\n      |> tableFind(fn: (key) => exists key._ci)\n      |> getRecord(idx: 0)\n  _level =\n    if exists _r0 then\n      getLevel(token: _r0._ci)\n    else\n       666\n  return _level\n}",
					Start: ast.Position{
						Column: 1,
						Line:   25,
					},
				},
			},
			ID: &ast.Identifier{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 15,
							Line:   25,
						},
						File:   "geo.flux",
						Source: "_detectCiLevel",
						Start: ast.Position{
							Column: 1,
							Line:   25,
						},
					},
				},
				Name: "_detectCiLevel",
			},
			Init: &ast.FunctionExpression{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 2,
							Line:   36,
						},
						File:   "geo.flux",
						Source: "(tables=<-) => {\n  _r0 =\n    tables\n      |> tableFind(fn: (key) => exists key._ci)\n      |> getRecord(idx: 0)\n  _level =\n    if exists _r0 then\n      getLevel(token: _r0._ci)\n    else\n       666\n  return _level\n}",
						Start: ast.Position{
							Column: 18,
							Line:   25,
						},
					},
				},
				Body: &ast.Block{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 2,
								Line:   36,
							},
							File:   "geo.flux",
							Source: "{\n  _r0 =\n    tables\n      |> tableFind(fn: (key) => exists key._ci)\n      |> getRecord(idx: 0)\n  _level =\n    if exists _r0 then\n      getLevel(token: _r0._ci)\n    else\n       666\n  return _level\n}",
							Start: ast.Position{
								Column: 33,
								Line:   25,
							},
						},
					},
					Body: []ast.Statement{&ast.VariableAssignment{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 27,
									Line:   29,
								},
								File:   "geo.flux",
								Source: "_r0 =\n    tables\n      |> tableFind(fn: (key) => exists key._ci)\n      |> getRecord(idx: 0)",
								Start: ast.Position{
									Column: 3,
									Line:   26,
								},
							},
						},
						ID: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 6,
										Line:   26,
									},
									File:   "geo.flux",
									Source: "_r0",
									Start: ast.Position{
										Column: 3,
										Line:   26,
									},
								},
							},
							Name: "_r0",
						},
						Init: &ast.PipeExpression{
							Argument: &ast.PipeExpression{
								Argument: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 11,
												Line:   27,
											},
											File:   "geo.flux",
											Source: "tables",
											Start: ast.Position{
												Column: 5,
												Line:   27,
											},
										},
									},
									Name: "tables",
								},
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 48,
											Line:   28,
										},
										File:   "geo.flux",
										Source: "tables\n      |> tableFind(fn: (key) => exists key._ci)",
										Start: ast.Position{
											Column: 5,
											Line:   27,
										},
									},
								},
								Call: &ast.CallExpression{
									Arguments: []ast.Expression{&ast.ObjectExpression{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 47,
													Line:   28,
												},
												File:   "geo.flux",
												Source: "fn: (key) => exists key._ci",
												Start: ast.Position{
													Column: 20,
													Line:   28,
												},
											},
										},
										Properties: []*ast.Property{&ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 47,
														Line:   28,
													},
													File:   "geo.flux",
													Source: "fn: (key) => exists key._ci",
													Start: ast.Position{
														Column: 20,
														Line:   28,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 22,
															Line:   28,
														},
														File:   "geo.flux",
														Source: "fn",
														Start: ast.Position{
															Column: 20,
															Line:   28,
														},
													},
												},
												Name: "fn",
											},
											Value: &ast.FunctionExpression{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 47,
															Line:   28,
														},
														File:   "geo.flux",
														Source: "(key) => exists key._ci",
														Start: ast.Position{
															Column: 24,
															Line:   28,
														},
													},
												},
												Body: &ast.UnaryExpression{
													Argument: &ast.MemberExpression{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 47,
																	Line:   28,
																},
																File:   "geo.flux",
																Source: "key._ci",
																Start: ast.Position{
																	Column: 40,
																	Line:   28,
																},
															},
														},
														Object: &ast.Identifier{
															BaseNode: ast.BaseNode{
																Errors: nil,
																Loc: &ast.SourceLocation{
																	End: ast.Position{
																		Column: 43,
																		Line:   28,
																	},
																	File:   "geo.flux",
																	Source: "key",
																	Start: ast.Position{
																		Column: 40,
																		Line:   28,
																	},
																},
															},
															Name: "key",
														},
														Property: &ast.Identifier{
															BaseNode: ast.BaseNode{
																Errors: nil,
																Loc: &ast.SourceLocation{
																	End: ast.Position{
																		Column: 47,
																		Line:   28,
																	},
																	File:   "geo.flux",
																	Source: "_ci",
																	Start: ast.Position{
																		Column: 44,
																		Line:   28,
																	},
																},
															},
															Name: "_ci",
														},
													},
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 47,
																Line:   28,
															},
															File:   "geo.flux",
															Source: "exists key._ci",
															Start: ast.Position{
																Column: 33,
																Line:   28,
															},
														},
													},
													Operator: 14,
												},
												Params: []*ast.Property{&ast.Property{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 28,
																Line:   28,
															},
															File:   "geo.flux",
															Source: "key",
															Start: ast.Position{
																Column: 25,
																Line:   28,
															},
														},
													},
													Key: &ast.Identifier{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 28,
																	Line:   28,
																},
																File:   "geo.flux",
																Source: "key",
																Start: ast.Position{
																	Column: 25,
																	Line:   28,
																},
															},
														},
														Name: "key",
													},
													Value: nil,
												}},
											},
										}},
										With: nil,
									}},
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 48,
												Line:   28,
											},
											File:   "geo.flux",
											Source: "tableFind(fn: (key) => exists key._ci)",
											Start: ast.Position{
												Column: 10,
												Line:   28,
											},
										},
									},
									Callee: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 19,
													Line:   28,
												},
												File:   "geo.flux",
												Source: "tableFind",
												Start: ast.Position{
													Column: 10,
													Line:   28,
												},
											},
										},
										Name: "tableFind",
									},
								},
							},
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 27,
										Line:   29,
									},
									File:   "geo.flux",
									Source: "tables\n      |> tableFind(fn: (key) => exists key._ci)\n      |> getRecord(idx: 0)",
									Start: ast.Position{
										Column: 5,
										Line:   27,
									},
								},
							},
							Call: &ast.CallExpression{
								Arguments: []ast.Expression{&ast.ObjectExpression{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 26,
												Line:   29,
											},
											File:   "geo.flux",
											Source: "idx: 0",
											Start: ast.Position{
												Column: 20,
												Line:   29,
											},
										},
									},
									Properties: []*ast.Property{&ast.Property{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 26,
													Line:   29,
												},
												File:   "geo.flux",
												Source: "idx: 0",
												Start: ast.Position{
													Column: 20,
													Line:   29,
												},
											},
										},
										Key: &ast.Identifier{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 23,
														Line:   29,
													},
													File:   "geo.flux",
													Source: "idx",
													Start: ast.Position{
														Column: 20,
														Line:   29,
													},
												},
											},
											Name: "idx",
										},
										Value: &ast.IntegerLiteral{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 26,
														Line:   29,
													},
													File:   "geo.flux",
													Source: "0",
													Start: ast.Position{
														Column: 25,
														Line:   29,
													},
												},
											},
											Value: int64(0),
										},
									}},
									With: nil,
								}},
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 27,
											Line:   29,
										},
										File:   "geo.flux",
										Source: "getRecord(idx: 0)",
										Start: ast.Position{
											Column: 10,
											Line:   29,
										},
									},
								},
								Callee: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 19,
												Line:   29,
											},
											File:   "geo.flux",
											Source: "getRecord",
											Start: ast.Position{
												Column: 10,
												Line:   29,
											},
										},
									},
									Name: "getRecord",
								},
							},
						},
					}, &ast.VariableAssignment{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 11,
									Line:   34,
								},
								File:   "geo.flux",
								Source: "_level =\n    if exists _r0 then\n      getLevel(token: _r0._ci)\n    else\n       666",
								Start: ast.Position{
									Column: 3,
									Line:   30,
								},
							},
						},
						ID: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 9,
										Line:   30,
									},
									File:   "geo.flux",
									Source: "_level",
									Start: ast.Position{
										Column: 3,
										Line:   30,
									},
								},
							},
							Name: "_level",
						},
						Init: &ast.ConditionalExpression{
							Alternate: &ast.IntegerLiteral{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 11,
											Line:   34,
										},
										File:   "geo.flux",
										Source: "666",
										Start: ast.Position{
											Column: 8,
											Line:   34,
										},
									},
								},
								Value: int64(666),
							},
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 11,
										Line:   34,
									},
									File:   "geo.flux",
									Source: "if exists _r0 then\n      getLevel(token: _r0._ci)\n    else\n       666",
									Start: ast.Position{
										Column: 5,
										Line:   31,
									},
								},
							},
							Consequent: &ast.CallExpression{
								Arguments: []ast.Expression{&ast.ObjectExpression{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 30,
												Line:   32,
											},
											File:   "geo.flux",
											Source: "token: _r0._ci",
											Start: ast.Position{
												Column: 16,
												Line:   32,
											},
										},
									},
									Properties: []*ast.Property{&ast.Property{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 30,
													Line:   32,
												},
												File:   "geo.flux",
												Source: "token: _r0._ci",
												Start: ast.Position{
													Column: 16,
													Line:   32,
												},
											},
										},
										Key: &ast.Identifier{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 21,
														Line:   32,
													},
													File:   "geo.flux",
													Source: "token",
													Start: ast.Position{
														Column: 16,
														Line:   32,
													},
												},
											},
											Name: "token",
										},
										Value: &ast.MemberExpression{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 30,
														Line:   32,
													},
													File:   "geo.flux",
													Source: "_r0._ci",
													Start: ast.Position{
														Column: 23,
														Line:   32,
													},
												},
											},
											Object: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 26,
															Line:   32,
														},
														File:   "geo.flux",
														Source: "_r0",
														Start: ast.Position{
															Column: 23,
															Line:   32,
														},
													},
												},
												Name: "_r0",
											},
											Property: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 30,
															Line:   32,
														},
														File:   "geo.flux",
														Source: "_ci",
														Start: ast.Position{
															Column: 27,
															Line:   32,
														},
													},
												},
												Name: "_ci",
											},
										},
									}},
									With: nil,
								}},
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 31,
											Line:   32,
										},
										File:   "geo.flux",
										Source: "getLevel(token: _r0._ci)",
										Start: ast.Position{
											Column: 7,
											Line:   32,
										},
									},
								},
								Callee: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 15,
												Line:   32,
											},
											File:   "geo.flux",
											Source: "getLevel",
											Start: ast.Position{
												Column: 7,
												Line:   32,
											},
										},
									},
									Name: "getLevel",
								},
							},
							Test: &ast.UnaryExpression{
								Argument: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 18,
												Line:   31,
											},
											File:   "geo.flux",
											Source: "_r0",
											Start: ast.Position{
												Column: 15,
												Line:   31,
											},
										},
									},
									Name: "_r0",
								},
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 18,
											Line:   31,
										},
										File:   "geo.flux",
										Source: "exists _r0",
										Start: ast.Position{
											Column: 8,
											Line:   31,
										},
									},
								},
								Operator: 14,
							},
						},
					}, &ast.ReturnStatement{
						Argument: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 16,
										Line:   35,
									},
									File:   "geo.flux",
									Source: "_level",
									Start: ast.Position{
										Column: 10,
										Line:   35,
									},
								},
							},
							Name: "_level",
						},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 16,
									Line:   35,
								},
								File:   "geo.flux",
								Source: "return _level",
								Start: ast.Position{
									Column: 3,
									Line:   35,
								},
							},
						},
					}},
				},
				Params: []*ast.Property{&ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 28,
								Line:   25,
							},
							File:   "geo.flux",
							Source: "tables=<-",
							Start: ast.Position{
								Column: 19,
								Line:   25,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 25,
									Line:   25,
								},
								File:   "geo.flux",
								Source: "tables",
								Start: ast.Position{
									Column: 19,
									Line:   25,
								},
							},
						},
						Name: "tables",
					},
					Value: &ast.PipeLiteral{BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 28,
								Line:   25,
							},
							File:   "geo.flux",
							Source: "<-",
							Start: ast.Position{
								Column: 26,
								Line:   25,
							},
						},
					}},
				}},
			},
		}, &ast.VariableAssignment{
			BaseNode: ast.BaseNode{
				Errors: nil,
				Loc: &ast.SourceLocation{
					End: ast.Position{
						Column: 6,
						Line:   49,
					},
					File:   "geo.flux",
					Source: "toRows = (tables=<-, correlationKey=[\"_time\"]) =>\n  tables\n    |> pivot(\n      rowKey: correlationKey,\n      columnKey: [\"_field\"],\n      valueColumn: \"_value\"\n    )",
					Start: ast.Position{
						Column: 1,
						Line:   43,
					},
				},
			},
			ID: &ast.Identifier{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 7,
							Line:   43,
						},
						File:   "geo.flux",
						Source: "toRows",
						Start: ast.Position{
							Column: 1,
							Line:   43,
						},
					},
				},
				Name: "toRows",
			},
			Init: &ast.FunctionExpression{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 6,
							Line:   49,
						},
						File:   "geo.flux",
						Source: "(tables=<-, correlationKey=[\"_time\"]) =>\n  tables\n    |> pivot(\n      rowKey: correlationKey,\n      columnKey: [\"_field\"],\n      valueColumn: \"_value\"\n    )",
						Start: ast.Position{
							Column: 10,
							Line:   43,
						},
					},
				},
				Body: &ast.PipeExpression{
					Argument: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 9,
									Line:   44,
								},
								File:   "geo.flux",
								Source: "tables",
								Start: ast.Position{
									Column: 3,
									Line:   44,
								},
							},
						},
						Name: "tables",
					},
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 6,
								Line:   49,
							},
							File:   "geo.flux",
							Source: "tables\n    |> pivot(\n      rowKey: correlationKey,\n      columnKey: [\"_field\"],\n      valueColumn: \"_value\"\n    )",
							Start: ast.Position{
								Column: 3,
								Line:   44,
							},
						},
					},
					Call: &ast.CallExpression{
						Arguments: []ast.Expression{&ast.ObjectExpression{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 28,
										Line:   48,
									},
									File:   "geo.flux",
									Source: "rowKey: correlationKey,\n      columnKey: [\"_field\"],\n      valueColumn: \"_value\"",
									Start: ast.Position{
										Column: 7,
										Line:   46,
									},
								},
							},
							Properties: []*ast.Property{&ast.Property{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 29,
											Line:   46,
										},
										File:   "geo.flux",
										Source: "rowKey: correlationKey",
										Start: ast.Position{
											Column: 7,
											Line:   46,
										},
									},
								},
								Key: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 13,
												Line:   46,
											},
											File:   "geo.flux",
											Source: "rowKey",
											Start: ast.Position{
												Column: 7,
												Line:   46,
											},
										},
									},
									Name: "rowKey",
								},
								Value: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 29,
												Line:   46,
											},
											File:   "geo.flux",
											Source: "correlationKey",
											Start: ast.Position{
												Column: 15,
												Line:   46,
											},
										},
									},
									Name: "correlationKey",
								},
							}, &ast.Property{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 28,
											Line:   47,
										},
										File:   "geo.flux",
										Source: "columnKey: [\"_field\"]",
										Start: ast.Position{
											Column: 7,
											Line:   47,
										},
									},
								},
								Key: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 16,
												Line:   47,
											},
											File:   "geo.flux",
											Source: "columnKey",
											Start: ast.Position{
												Column: 7,
												Line:   47,
											},
										},
									},
									Name: "columnKey",
								},
								Value: &ast.ArrayExpression{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 28,
												Line:   47,
											},
											File:   "geo.flux",
											Source: "[\"_field\"]",
											Start: ast.Position{
												Column: 18,
												Line:   47,
											},
										},
									},
									Elements: []ast.Expression{&ast.StringLiteral{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 27,
													Line:   47,
												},
												File:   "geo.flux",
												Source: "\"_field\"",
												Start: ast.Position{
													Column: 19,
													Line:   47,
												},
											},
										},
										Value: "_field",
									}},
								},
							}, &ast.Property{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 28,
											Line:   48,
										},
										File:   "geo.flux",
										Source: "valueColumn: \"_value\"",
										Start: ast.Position{
											Column: 7,
											Line:   48,
										},
									},
								},
								Key: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 18,
												Line:   48,
											},
											File:   "geo.flux",
											Source: "valueColumn",
											Start: ast.Position{
												Column: 7,
												Line:   48,
											},
										},
									},
									Name: "valueColumn",
								},
								Value: &ast.StringLiteral{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 28,
												Line:   48,
											},
											File:   "geo.flux",
											Source: "\"_value\"",
											Start: ast.Position{
												Column: 20,
												Line:   48,
											},
										},
									},
									Value: "_value",
								},
							}},
							With: nil,
						}},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 6,
									Line:   49,
								},
								File:   "geo.flux",
								Source: "pivot(\n      rowKey: correlationKey,\n      columnKey: [\"_field\"],\n      valueColumn: \"_value\"\n    )",
								Start: ast.Position{
									Column: 8,
									Line:   45,
								},
							},
						},
						Callee: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 13,
										Line:   45,
									},
									File:   "geo.flux",
									Source: "pivot",
									Start: ast.Position{
										Column: 8,
										Line:   45,
									},
								},
							},
							Name: "pivot",
						},
					},
				},
				Params: []*ast.Property{&ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 20,
								Line:   43,
							},
							File:   "geo.flux",
							Source: "tables=<-",
							Start: ast.Position{
								Column: 11,
								Line:   43,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 17,
									Line:   43,
								},
								File:   "geo.flux",
								Source: "tables",
								Start: ast.Position{
									Column: 11,
									Line:   43,
								},
							},
						},
						Name: "tables",
					},
					Value: &ast.PipeLiteral{BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 20,
								Line:   43,
							},
							File:   "geo.flux",
							Source: "<-",
							Start: ast.Position{
								Column: 18,
								Line:   43,
							},
						},
					}},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 46,
								Line:   43,
							},
							File:   "geo.flux",
							Source: "correlationKey=[\"_time\"]",
							Start: ast.Position{
								Column: 22,
								Line:   43,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 36,
									Line:   43,
								},
								File:   "geo.flux",
								Source: "correlationKey",
								Start: ast.Position{
									Column: 22,
									Line:   43,
								},
							},
						},
						Name: "correlationKey",
					},
					Value: &ast.ArrayExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 46,
									Line:   43,
								},
								File:   "geo.flux",
								Source: "[\"_time\"]",
								Start: ast.Position{
									Column: 37,
									Line:   43,
								},
							},
						},
						Elements: []ast.Expression{&ast.StringLiteral{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 45,
										Line:   43,
									},
									File:   "geo.flux",
									Source: "\"_time\"",
									Start: ast.Position{
										Column: 38,
										Line:   43,
									},
								},
							},
							Value: "_time",
						}},
					},
				}},
			},
		}, &ast.VariableAssignment{
			BaseNode: ast.BaseNode{
				Errors: nil,
				Loc: &ast.SourceLocation{
					End: ast.Position{
						Column: 2,
						Line:   74,
					},
					File:   "geo.flux",
					Source: "gridFilter = (tables=<-, box={}, circle={}, polygon={}, minSize=24, maxSize=-1, level=-1, ciLevel=-1) => {\n  _ciLevel =\n    if ciLevel == -1 then\n      tables\n        |> _detectCiLevel()\n    else\n      ciLevel\n  _grid = getGrid(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, maxLevel: _ciLevel)\n  return\n    tables\n      |> filter(fn: (r) =>\n        if _grid.level == _ciLevel then\n          contains(value: r._ci, set: _grid.set)\n        else\n          contains(value: getParent(token: r._ci, level: _grid.level), set: _grid.set)\n      )\n}",
					Start: ast.Position{
						Column: 1,
						Line:   58,
					},
				},
			},
			ID: &ast.Identifier{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 11,
							Line:   58,
						},
						File:   "geo.flux",
						Source: "gridFilter",
						Start: ast.Position{
							Column: 1,
							Line:   58,
						},
					},
				},
				Name: "gridFilter",
			},
			Init: &ast.FunctionExpression{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 2,
							Line:   74,
						},
						File:   "geo.flux",
						Source: "(tables=<-, box={}, circle={}, polygon={}, minSize=24, maxSize=-1, level=-1, ciLevel=-1) => {\n  _ciLevel =\n    if ciLevel == -1 then\n      tables\n        |> _detectCiLevel()\n    else\n      ciLevel\n  _grid = getGrid(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, maxLevel: _ciLevel)\n  return\n    tables\n      |> filter(fn: (r) =>\n        if _grid.level == _ciLevel then\n          contains(value: r._ci, set: _grid.set)\n        else\n          contains(value: getParent(token: r._ci, level: _grid.level), set: _grid.set)\n      )\n}",
						Start: ast.Position{
							Column: 14,
							Line:   58,
						},
					},
				},
				Body: &ast.Block{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 2,
								Line:   74,
							},
							File:   "geo.flux",
							Source: "{\n  _ciLevel =\n    if ciLevel == -1 then\n      tables\n        |> _detectCiLevel()\n    else\n      ciLevel\n  _grid = getGrid(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, maxLevel: _ciLevel)\n  return\n    tables\n      |> filter(fn: (r) =>\n        if _grid.level == _ciLevel then\n          contains(value: r._ci, set: _grid.set)\n        else\n          contains(value: getParent(token: r._ci, level: _grid.level), set: _grid.set)\n      )\n}",
							Start: ast.Position{
								Column: 106,
								Line:   58,
							},
						},
					},
					Body: []ast.Statement{&ast.VariableAssignment{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 14,
									Line:   64,
								},
								File:   "geo.flux",
								Source: "_ciLevel =\n    if ciLevel == -1 then\n      tables\n        |> _detectCiLevel()\n    else\n      ciLevel",
								Start: ast.Position{
									Column: 3,
									Line:   59,
								},
							},
						},
						ID: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 11,
										Line:   59,
									},
									File:   "geo.flux",
									Source: "_ciLevel",
									Start: ast.Position{
										Column: 3,
										Line:   59,
									},
								},
							},
							Name: "_ciLevel",
						},
						Init: &ast.ConditionalExpression{
							Alternate: &ast.Identifier{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 14,
											Line:   64,
										},
										File:   "geo.flux",
										Source: "ciLevel",
										Start: ast.Position{
											Column: 7,
											Line:   64,
										},
									},
								},
								Name: "ciLevel",
							},
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 14,
										Line:   64,
									},
									File:   "geo.flux",
									Source: "if ciLevel == -1 then\n      tables\n        |> _detectCiLevel()\n    else\n      ciLevel",
									Start: ast.Position{
										Column: 5,
										Line:   60,
									},
								},
							},
							Consequent: &ast.PipeExpression{
								Argument: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 13,
												Line:   61,
											},
											File:   "geo.flux",
											Source: "tables",
											Start: ast.Position{
												Column: 7,
												Line:   61,
											},
										},
									},
									Name: "tables",
								},
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 28,
											Line:   62,
										},
										File:   "geo.flux",
										Source: "tables\n        |> _detectCiLevel()",
										Start: ast.Position{
											Column: 7,
											Line:   61,
										},
									},
								},
								Call: &ast.CallExpression{
									Arguments: nil,
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 28,
												Line:   62,
											},
											File:   "geo.flux",
											Source: "_detectCiLevel()",
											Start: ast.Position{
												Column: 12,
												Line:   62,
											},
										},
									},
									Callee: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 26,
													Line:   62,
												},
												File:   "geo.flux",
												Source: "_detectCiLevel",
												Start: ast.Position{
													Column: 12,
													Line:   62,
												},
											},
										},
										Name: "_detectCiLevel",
									},
								},
							},
							Test: &ast.BinaryExpression{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 21,
											Line:   60,
										},
										File:   "geo.flux",
										Source: "ciLevel == -1",
										Start: ast.Position{
											Column: 8,
											Line:   60,
										},
									},
								},
								Left: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 15,
												Line:   60,
											},
											File:   "geo.flux",
											Source: "ciLevel",
											Start: ast.Position{
												Column: 8,
												Line:   60,
											},
										},
									},
									Name: "ciLevel",
								},
								Operator: 17,
								Right: &ast.UnaryExpression{
									Argument: &ast.IntegerLiteral{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 21,
													Line:   60,
												},
												File:   "geo.flux",
												Source: "1",
												Start: ast.Position{
													Column: 20,
													Line:   60,
												},
											},
										},
										Value: int64(1),
									},
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 21,
												Line:   60,
											},
											File:   "geo.flux",
											Source: "-1",
											Start: ast.Position{
												Column: 19,
												Line:   60,
											},
										},
									},
									Operator: 6,
								},
							},
						},
					}, &ast.VariableAssignment{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 132,
									Line:   65,
								},
								File:   "geo.flux",
								Source: "_grid = getGrid(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, maxLevel: _ciLevel)",
								Start: ast.Position{
									Column: 3,
									Line:   65,
								},
							},
						},
						ID: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 8,
										Line:   65,
									},
									File:   "geo.flux",
									Source: "_grid",
									Start: ast.Position{
										Column: 3,
										Line:   65,
									},
								},
							},
							Name: "_grid",
						},
						Init: &ast.CallExpression{
							Arguments: []ast.Expression{&ast.ObjectExpression{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 131,
											Line:   65,
										},
										File:   "geo.flux",
										Source: "box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, maxLevel: _ciLevel",
										Start: ast.Position{
											Column: 19,
											Line:   65,
										},
									},
								},
								Properties: []*ast.Property{&ast.Property{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 27,
												Line:   65,
											},
											File:   "geo.flux",
											Source: "box: box",
											Start: ast.Position{
												Column: 19,
												Line:   65,
											},
										},
									},
									Key: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 22,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "box",
												Start: ast.Position{
													Column: 19,
													Line:   65,
												},
											},
										},
										Name: "box",
									},
									Value: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 27,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "box",
												Start: ast.Position{
													Column: 24,
													Line:   65,
												},
											},
										},
										Name: "box",
									},
								}, &ast.Property{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 43,
												Line:   65,
											},
											File:   "geo.flux",
											Source: "circle: circle",
											Start: ast.Position{
												Column: 29,
												Line:   65,
											},
										},
									},
									Key: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 35,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "circle",
												Start: ast.Position{
													Column: 29,
													Line:   65,
												},
											},
										},
										Name: "circle",
									},
									Value: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 43,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "circle",
												Start: ast.Position{
													Column: 37,
													Line:   65,
												},
											},
										},
										Name: "circle",
									},
								}, &ast.Property{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 61,
												Line:   65,
											},
											File:   "geo.flux",
											Source: "polygon: polygon",
											Start: ast.Position{
												Column: 45,
												Line:   65,
											},
										},
									},
									Key: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 52,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "polygon",
												Start: ast.Position{
													Column: 45,
													Line:   65,
												},
											},
										},
										Name: "polygon",
									},
									Value: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 61,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "polygon",
												Start: ast.Position{
													Column: 54,
													Line:   65,
												},
											},
										},
										Name: "polygon",
									},
								}, &ast.Property{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 79,
												Line:   65,
											},
											File:   "geo.flux",
											Source: "minSize: minSize",
											Start: ast.Position{
												Column: 63,
												Line:   65,
											},
										},
									},
									Key: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 70,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "minSize",
												Start: ast.Position{
													Column: 63,
													Line:   65,
												},
											},
										},
										Name: "minSize",
									},
									Value: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 79,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "minSize",
												Start: ast.Position{
													Column: 72,
													Line:   65,
												},
											},
										},
										Name: "minSize",
									},
								}, &ast.Property{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 97,
												Line:   65,
											},
											File:   "geo.flux",
											Source: "maxSize: maxSize",
											Start: ast.Position{
												Column: 81,
												Line:   65,
											},
										},
									},
									Key: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 88,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "maxSize",
												Start: ast.Position{
													Column: 81,
													Line:   65,
												},
											},
										},
										Name: "maxSize",
									},
									Value: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 97,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "maxSize",
												Start: ast.Position{
													Column: 90,
													Line:   65,
												},
											},
										},
										Name: "maxSize",
									},
								}, &ast.Property{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 111,
												Line:   65,
											},
											File:   "geo.flux",
											Source: "level: level",
											Start: ast.Position{
												Column: 99,
												Line:   65,
											},
										},
									},
									Key: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 104,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "level",
												Start: ast.Position{
													Column: 99,
													Line:   65,
												},
											},
										},
										Name: "level",
									},
									Value: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 111,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "level",
												Start: ast.Position{
													Column: 106,
													Line:   65,
												},
											},
										},
										Name: "level",
									},
								}, &ast.Property{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 131,
												Line:   65,
											},
											File:   "geo.flux",
											Source: "maxLevel: _ciLevel",
											Start: ast.Position{
												Column: 113,
												Line:   65,
											},
										},
									},
									Key: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 121,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "maxLevel",
												Start: ast.Position{
													Column: 113,
													Line:   65,
												},
											},
										},
										Name: "maxLevel",
									},
									Value: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 131,
													Line:   65,
												},
												File:   "geo.flux",
												Source: "_ciLevel",
												Start: ast.Position{
													Column: 123,
													Line:   65,
												},
											},
										},
										Name: "_ciLevel",
									},
								}},
								With: nil,
							}},
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 132,
										Line:   65,
									},
									File:   "geo.flux",
									Source: "getGrid(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, maxLevel: _ciLevel)",
									Start: ast.Position{
										Column: 11,
										Line:   65,
									},
								},
							},
							Callee: &ast.Identifier{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 18,
											Line:   65,
										},
										File:   "geo.flux",
										Source: "getGrid",
										Start: ast.Position{
											Column: 11,
											Line:   65,
										},
									},
								},
								Name: "getGrid",
							},
						},
					}, &ast.ReturnStatement{
						Argument: &ast.PipeExpression{
							Argument: &ast.Identifier{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 11,
											Line:   67,
										},
										File:   "geo.flux",
										Source: "tables",
										Start: ast.Position{
											Column: 5,
											Line:   67,
										},
									},
								},
								Name: "tables",
							},
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 8,
										Line:   73,
									},
									File:   "geo.flux",
									Source: "tables\n      |> filter(fn: (r) =>\n        if _grid.level == _ciLevel then\n          contains(value: r._ci, set: _grid.set)\n        else\n          contains(value: getParent(token: r._ci, level: _grid.level), set: _grid.set)\n      )",
									Start: ast.Position{
										Column: 5,
										Line:   67,
									},
								},
							},
							Call: &ast.CallExpression{
								Arguments: []ast.Expression{&ast.ObjectExpression{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 87,
												Line:   72,
											},
											File:   "geo.flux",
											Source: "fn: (r) =>\n        if _grid.level == _ciLevel then\n          contains(value: r._ci, set: _grid.set)\n        else\n          contains(value: getParent(token: r._ci, level: _grid.level), set: _grid.set)",
											Start: ast.Position{
												Column: 17,
												Line:   68,
											},
										},
									},
									Properties: []*ast.Property{&ast.Property{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 87,
													Line:   72,
												},
												File:   "geo.flux",
												Source: "fn: (r) =>\n        if _grid.level == _ciLevel then\n          contains(value: r._ci, set: _grid.set)\n        else\n          contains(value: getParent(token: r._ci, level: _grid.level), set: _grid.set)",
												Start: ast.Position{
													Column: 17,
													Line:   68,
												},
											},
										},
										Key: &ast.Identifier{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 19,
														Line:   68,
													},
													File:   "geo.flux",
													Source: "fn",
													Start: ast.Position{
														Column: 17,
														Line:   68,
													},
												},
											},
											Name: "fn",
										},
										Value: &ast.FunctionExpression{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 87,
														Line:   72,
													},
													File:   "geo.flux",
													Source: "(r) =>\n        if _grid.level == _ciLevel then\n          contains(value: r._ci, set: _grid.set)\n        else\n          contains(value: getParent(token: r._ci, level: _grid.level), set: _grid.set)",
													Start: ast.Position{
														Column: 21,
														Line:   68,
													},
												},
											},
											Body: &ast.ConditionalExpression{
												Alternate: &ast.CallExpression{
													Arguments: []ast.Expression{&ast.ObjectExpression{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 86,
																	Line:   72,
																},
																File:   "geo.flux",
																Source: "value: getParent(token: r._ci, level: _grid.level), set: _grid.set",
																Start: ast.Position{
																	Column: 20,
																	Line:   72,
																},
															},
														},
														Properties: []*ast.Property{&ast.Property{
															BaseNode: ast.BaseNode{
																Errors: nil,
																Loc: &ast.SourceLocation{
																	End: ast.Position{
																		Column: 70,
																		Line:   72,
																	},
																	File:   "geo.flux",
																	Source: "value: getParent(token: r._ci, level: _grid.level)",
																	Start: ast.Position{
																		Column: 20,
																		Line:   72,
																	},
																},
															},
															Key: &ast.Identifier{
																BaseNode: ast.BaseNode{
																	Errors: nil,
																	Loc: &ast.SourceLocation{
																		End: ast.Position{
																			Column: 25,
																			Line:   72,
																		},
																		File:   "geo.flux",
																		Source: "value",
																		Start: ast.Position{
																			Column: 20,
																			Line:   72,
																		},
																	},
																},
																Name: "value",
															},
															Value: &ast.CallExpression{
																Arguments: []ast.Expression{&ast.ObjectExpression{
																	BaseNode: ast.BaseNode{
																		Errors: nil,
																		Loc: &ast.SourceLocation{
																			End: ast.Position{
																				Column: 69,
																				Line:   72,
																			},
																			File:   "geo.flux",
																			Source: "token: r._ci, level: _grid.level",
																			Start: ast.Position{
																				Column: 37,
																				Line:   72,
																			},
																		},
																	},
																	Properties: []*ast.Property{&ast.Property{
																		BaseNode: ast.BaseNode{
																			Errors: nil,
																			Loc: &ast.SourceLocation{
																				End: ast.Position{
																					Column: 49,
																					Line:   72,
																				},
																				File:   "geo.flux",
																				Source: "token: r._ci",
																				Start: ast.Position{
																					Column: 37,
																					Line:   72,
																				},
																			},
																		},
																		Key: &ast.Identifier{
																			BaseNode: ast.BaseNode{
																				Errors: nil,
																				Loc: &ast.SourceLocation{
																					End: ast.Position{
																						Column: 42,
																						Line:   72,
																					},
																					File:   "geo.flux",
																					Source: "token",
																					Start: ast.Position{
																						Column: 37,
																						Line:   72,
																					},
																				},
																			},
																			Name: "token",
																		},
																		Value: &ast.MemberExpression{
																			BaseNode: ast.BaseNode{
																				Errors: nil,
																				Loc: &ast.SourceLocation{
																					End: ast.Position{
																						Column: 49,
																						Line:   72,
																					},
																					File:   "geo.flux",
																					Source: "r._ci",
																					Start: ast.Position{
																						Column: 44,
																						Line:   72,
																					},
																				},
																			},
																			Object: &ast.Identifier{
																				BaseNode: ast.BaseNode{
																					Errors: nil,
																					Loc: &ast.SourceLocation{
																						End: ast.Position{
																							Column: 45,
																							Line:   72,
																						},
																						File:   "geo.flux",
																						Source: "r",
																						Start: ast.Position{
																							Column: 44,
																							Line:   72,
																						},
																					},
																				},
																				Name: "r",
																			},
																			Property: &ast.Identifier{
																				BaseNode: ast.BaseNode{
																					Errors: nil,
																					Loc: &ast.SourceLocation{
																						End: ast.Position{
																							Column: 49,
																							Line:   72,
																						},
																						File:   "geo.flux",
																						Source: "_ci",
																						Start: ast.Position{
																							Column: 46,
																							Line:   72,
																						},
																					},
																				},
																				Name: "_ci",
																			},
																		},
																	}, &ast.Property{
																		BaseNode: ast.BaseNode{
																			Errors: nil,
																			Loc: &ast.SourceLocation{
																				End: ast.Position{
																					Column: 69,
																					Line:   72,
																				},
																				File:   "geo.flux",
																				Source: "level: _grid.level",
																				Start: ast.Position{
																					Column: 51,
																					Line:   72,
																				},
																			},
																		},
																		Key: &ast.Identifier{
																			BaseNode: ast.BaseNode{
																				Errors: nil,
																				Loc: &ast.SourceLocation{
																					End: ast.Position{
																						Column: 56,
																						Line:   72,
																					},
																					File:   "geo.flux",
																					Source: "level",
																					Start: ast.Position{
																						Column: 51,
																						Line:   72,
																					},
																				},
																			},
																			Name: "level",
																		},
																		Value: &ast.MemberExpression{
																			BaseNode: ast.BaseNode{
																				Errors: nil,
																				Loc: &ast.SourceLocation{
																					End: ast.Position{
																						Column: 69,
																						Line:   72,
																					},
																					File:   "geo.flux",
																					Source: "_grid.level",
																					Start: ast.Position{
																						Column: 58,
																						Line:   72,
																					},
																				},
																			},
																			Object: &ast.Identifier{
																				BaseNode: ast.BaseNode{
																					Errors: nil,
																					Loc: &ast.SourceLocation{
																						End: ast.Position{
																							Column: 63,
																							Line:   72,
																						},
																						File:   "geo.flux",
																						Source: "_grid",
																						Start: ast.Position{
																							Column: 58,
																							Line:   72,
																						},
																					},
																				},
																				Name: "_grid",
																			},
																			Property: &ast.Identifier{
																				BaseNode: ast.BaseNode{
																					Errors: nil,
																					Loc: &ast.SourceLocation{
																						End: ast.Position{
																							Column: 69,
																							Line:   72,
																						},
																						File:   "geo.flux",
																						Source: "level",
																						Start: ast.Position{
																							Column: 64,
																							Line:   72,
																						},
																					},
																				},
																				Name: "level",
																			},
																		},
																	}},
																	With: nil,
																}},
																BaseNode: ast.BaseNode{
																	Errors: nil,
																	Loc: &ast.SourceLocation{
																		End: ast.Position{
																			Column: 70,
																			Line:   72,
																		},
																		File:   "geo.flux",
																		Source: "getParent(token: r._ci, level: _grid.level)",
																		Start: ast.Position{
																			Column: 27,
																			Line:   72,
																		},
																	},
																},
																Callee: &ast.Identifier{
																	BaseNode: ast.BaseNode{
																		Errors: nil,
																		Loc: &ast.SourceLocation{
																			End: ast.Position{
																				Column: 36,
																				Line:   72,
																			},
																			File:   "geo.flux",
																			Source: "getParent",
																			Start: ast.Position{
																				Column: 27,
																				Line:   72,
																			},
																		},
																	},
																	Name: "getParent",
																},
															},
														}, &ast.Property{
															BaseNode: ast.BaseNode{
																Errors: nil,
																Loc: &ast.SourceLocation{
																	End: ast.Position{
																		Column: 86,
																		Line:   72,
																	},
																	File:   "geo.flux",
																	Source: "set: _grid.set",
																	Start: ast.Position{
																		Column: 72,
																		Line:   72,
																	},
																},
															},
															Key: &ast.Identifier{
																BaseNode: ast.BaseNode{
																	Errors: nil,
																	Loc: &ast.SourceLocation{
																		End: ast.Position{
																			Column: 75,
																			Line:   72,
																		},
																		File:   "geo.flux",
																		Source: "set",
																		Start: ast.Position{
																			Column: 72,
																			Line:   72,
																		},
																	},
																},
																Name: "set",
															},
															Value: &ast.MemberExpression{
																BaseNode: ast.BaseNode{
																	Errors: nil,
																	Loc: &ast.SourceLocation{
																		End: ast.Position{
																			Column: 86,
																			Line:   72,
																		},
																		File:   "geo.flux",
																		Source: "_grid.set",
																		Start: ast.Position{
																			Column: 77,
																			Line:   72,
																		},
																	},
																},
																Object: &ast.Identifier{
																	BaseNode: ast.BaseNode{
																		Errors: nil,
																		Loc: &ast.SourceLocation{
																			End: ast.Position{
																				Column: 82,
																				Line:   72,
																			},
																			File:   "geo.flux",
																			Source: "_grid",
																			Start: ast.Position{
																				Column: 77,
																				Line:   72,
																			},
																		},
																	},
																	Name: "_grid",
																},
																Property: &ast.Identifier{
																	BaseNode: ast.BaseNode{
																		Errors: nil,
																		Loc: &ast.SourceLocation{
																			End: ast.Position{
																				Column: 86,
																				Line:   72,
																			},
																			File:   "geo.flux",
																			Source: "set",
																			Start: ast.Position{
																				Column: 83,
																				Line:   72,
																			},
																		},
																	},
																	Name: "set",
																},
															},
														}},
														With: nil,
													}},
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 87,
																Line:   72,
															},
															File:   "geo.flux",
															Source: "contains(value: getParent(token: r._ci, level: _grid.level), set: _grid.set)",
															Start: ast.Position{
																Column: 11,
																Line:   72,
															},
														},
													},
													Callee: &ast.Identifier{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 19,
																	Line:   72,
																},
																File:   "geo.flux",
																Source: "contains",
																Start: ast.Position{
																	Column: 11,
																	Line:   72,
																},
															},
														},
														Name: "contains",
													},
												},
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 87,
															Line:   72,
														},
														File:   "geo.flux",
														Source: "if _grid.level == _ciLevel then\n          contains(value: r._ci, set: _grid.set)\n        else\n          contains(value: getParent(token: r._ci, level: _grid.level), set: _grid.set)",
														Start: ast.Position{
															Column: 9,
															Line:   69,
														},
													},
												},
												Consequent: &ast.CallExpression{
													Arguments: []ast.Expression{&ast.ObjectExpression{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 48,
																	Line:   70,
																},
																File:   "geo.flux",
																Source: "value: r._ci, set: _grid.set",
																Start: ast.Position{
																	Column: 20,
																	Line:   70,
																},
															},
														},
														Properties: []*ast.Property{&ast.Property{
															BaseNode: ast.BaseNode{
																Errors: nil,
																Loc: &ast.SourceLocation{
																	End: ast.Position{
																		Column: 32,
																		Line:   70,
																	},
																	File:   "geo.flux",
																	Source: "value: r._ci",
																	Start: ast.Position{
																		Column: 20,
																		Line:   70,
																	},
																},
															},
															Key: &ast.Identifier{
																BaseNode: ast.BaseNode{
																	Errors: nil,
																	Loc: &ast.SourceLocation{
																		End: ast.Position{
																			Column: 25,
																			Line:   70,
																		},
																		File:   "geo.flux",
																		Source: "value",
																		Start: ast.Position{
																			Column: 20,
																			Line:   70,
																		},
																	},
																},
																Name: "value",
															},
															Value: &ast.MemberExpression{
																BaseNode: ast.BaseNode{
																	Errors: nil,
																	Loc: &ast.SourceLocation{
																		End: ast.Position{
																			Column: 32,
																			Line:   70,
																		},
																		File:   "geo.flux",
																		Source: "r._ci",
																		Start: ast.Position{
																			Column: 27,
																			Line:   70,
																		},
																	},
																},
																Object: &ast.Identifier{
																	BaseNode: ast.BaseNode{
																		Errors: nil,
																		Loc: &ast.SourceLocation{
																			End: ast.Position{
																				Column: 28,
																				Line:   70,
																			},
																			File:   "geo.flux",
																			Source: "r",
																			Start: ast.Position{
																				Column: 27,
																				Line:   70,
																			},
																		},
																	},
																	Name: "r",
																},
																Property: &ast.Identifier{
																	BaseNode: ast.BaseNode{
																		Errors: nil,
																		Loc: &ast.SourceLocation{
																			End: ast.Position{
																				Column: 32,
																				Line:   70,
																			},
																			File:   "geo.flux",
																			Source: "_ci",
																			Start: ast.Position{
																				Column: 29,
																				Line:   70,
																			},
																		},
																	},
																	Name: "_ci",
																},
															},
														}, &ast.Property{
															BaseNode: ast.BaseNode{
																Errors: nil,
																Loc: &ast.SourceLocation{
																	End: ast.Position{
																		Column: 48,
																		Line:   70,
																	},
																	File:   "geo.flux",
																	Source: "set: _grid.set",
																	Start: ast.Position{
																		Column: 34,
																		Line:   70,
																	},
																},
															},
															Key: &ast.Identifier{
																BaseNode: ast.BaseNode{
																	Errors: nil,
																	Loc: &ast.SourceLocation{
																		End: ast.Position{
																			Column: 37,
																			Line:   70,
																		},
																		File:   "geo.flux",
																		Source: "set",
																		Start: ast.Position{
																			Column: 34,
																			Line:   70,
																		},
																	},
																},
																Name: "set",
															},
															Value: &ast.MemberExpression{
																BaseNode: ast.BaseNode{
																	Errors: nil,
																	Loc: &ast.SourceLocation{
																		End: ast.Position{
																			Column: 48,
																			Line:   70,
																		},
																		File:   "geo.flux",
																		Source: "_grid.set",
																		Start: ast.Position{
																			Column: 39,
																			Line:   70,
																		},
																	},
																},
																Object: &ast.Identifier{
																	BaseNode: ast.BaseNode{
																		Errors: nil,
																		Loc: &ast.SourceLocation{
																			End: ast.Position{
																				Column: 44,
																				Line:   70,
																			},
																			File:   "geo.flux",
																			Source: "_grid",
																			Start: ast.Position{
																				Column: 39,
																				Line:   70,
																			},
																		},
																	},
																	Name: "_grid",
																},
																Property: &ast.Identifier{
																	BaseNode: ast.BaseNode{
																		Errors: nil,
																		Loc: &ast.SourceLocation{
																			End: ast.Position{
																				Column: 48,
																				Line:   70,
																			},
																			File:   "geo.flux",
																			Source: "set",
																			Start: ast.Position{
																				Column: 45,
																				Line:   70,
																			},
																		},
																	},
																	Name: "set",
																},
															},
														}},
														With: nil,
													}},
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 49,
																Line:   70,
															},
															File:   "geo.flux",
															Source: "contains(value: r._ci, set: _grid.set)",
															Start: ast.Position{
																Column: 11,
																Line:   70,
															},
														},
													},
													Callee: &ast.Identifier{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 19,
																	Line:   70,
																},
																File:   "geo.flux",
																Source: "contains",
																Start: ast.Position{
																	Column: 11,
																	Line:   70,
																},
															},
														},
														Name: "contains",
													},
												},
												Test: &ast.BinaryExpression{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 35,
																Line:   69,
															},
															File:   "geo.flux",
															Source: "_grid.level == _ciLevel",
															Start: ast.Position{
																Column: 12,
																Line:   69,
															},
														},
													},
													Left: &ast.MemberExpression{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 23,
																	Line:   69,
																},
																File:   "geo.flux",
																Source: "_grid.level",
																Start: ast.Position{
																	Column: 12,
																	Line:   69,
																},
															},
														},
														Object: &ast.Identifier{
															BaseNode: ast.BaseNode{
																Errors: nil,
																Loc: &ast.SourceLocation{
																	End: ast.Position{
																		Column: 17,
																		Line:   69,
																	},
																	File:   "geo.flux",
																	Source: "_grid",
																	Start: ast.Position{
																		Column: 12,
																		Line:   69,
																	},
																},
															},
															Name: "_grid",
														},
														Property: &ast.Identifier{
															BaseNode: ast.BaseNode{
																Errors: nil,
																Loc: &ast.SourceLocation{
																	End: ast.Position{
																		Column: 23,
																		Line:   69,
																	},
																	File:   "geo.flux",
																	Source: "level",
																	Start: ast.Position{
																		Column: 18,
																		Line:   69,
																	},
																},
															},
															Name: "level",
														},
													},
													Operator: 17,
													Right: &ast.Identifier{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 35,
																	Line:   69,
																},
																File:   "geo.flux",
																Source: "_ciLevel",
																Start: ast.Position{
																	Column: 27,
																	Line:   69,
																},
															},
														},
														Name: "_ciLevel",
													},
												},
											},
											Params: []*ast.Property{&ast.Property{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 23,
															Line:   68,
														},
														File:   "geo.flux",
														Source: "r",
														Start: ast.Position{
															Column: 22,
															Line:   68,
														},
													},
												},
												Key: &ast.Identifier{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 23,
																Line:   68,
															},
															File:   "geo.flux",
															Source: "r",
															Start: ast.Position{
																Column: 22,
																Line:   68,
															},
														},
													},
													Name: "r",
												},
												Value: nil,
											}},
										},
									}},
									With: nil,
								}},
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 8,
											Line:   73,
										},
										File:   "geo.flux",
										Source: "filter(fn: (r) =>\n        if _grid.level == _ciLevel then\n          contains(value: r._ci, set: _grid.set)\n        else\n          contains(value: getParent(token: r._ci, level: _grid.level), set: _grid.set)\n      )",
										Start: ast.Position{
											Column: 10,
											Line:   68,
										},
									},
								},
								Callee: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 16,
												Line:   68,
											},
											File:   "geo.flux",
											Source: "filter",
											Start: ast.Position{
												Column: 10,
												Line:   68,
											},
										},
									},
									Name: "filter",
								},
							},
						},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 8,
									Line:   73,
								},
								File:   "geo.flux",
								Source: "return\n    tables\n      |> filter(fn: (r) =>\n        if _grid.level == _ciLevel then\n          contains(value: r._ci, set: _grid.set)\n        else\n          contains(value: getParent(token: r._ci, level: _grid.level), set: _grid.set)\n      )",
								Start: ast.Position{
									Column: 3,
									Line:   66,
								},
							},
						},
					}},
				},
				Params: []*ast.Property{&ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 24,
								Line:   58,
							},
							File:   "geo.flux",
							Source: "tables=<-",
							Start: ast.Position{
								Column: 15,
								Line:   58,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 21,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "tables",
								Start: ast.Position{
									Column: 15,
									Line:   58,
								},
							},
						},
						Name: "tables",
					},
					Value: &ast.PipeLiteral{BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 24,
								Line:   58,
							},
							File:   "geo.flux",
							Source: "<-",
							Start: ast.Position{
								Column: 22,
								Line:   58,
							},
						},
					}},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 32,
								Line:   58,
							},
							File:   "geo.flux",
							Source: "box={}",
							Start: ast.Position{
								Column: 26,
								Line:   58,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 29,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "box",
								Start: ast.Position{
									Column: 26,
									Line:   58,
								},
							},
						},
						Name: "box",
					},
					Value: &ast.ObjectExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 32,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "{}",
								Start: ast.Position{
									Column: 30,
									Line:   58,
								},
							},
						},
						Properties: nil,
						With:       nil,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 43,
								Line:   58,
							},
							File:   "geo.flux",
							Source: "circle={}",
							Start: ast.Position{
								Column: 34,
								Line:   58,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 40,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "circle",
								Start: ast.Position{
									Column: 34,
									Line:   58,
								},
							},
						},
						Name: "circle",
					},
					Value: &ast.ObjectExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 43,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "{}",
								Start: ast.Position{
									Column: 41,
									Line:   58,
								},
							},
						},
						Properties: nil,
						With:       nil,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 55,
								Line:   58,
							},
							File:   "geo.flux",
							Source: "polygon={}",
							Start: ast.Position{
								Column: 45,
								Line:   58,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 52,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "polygon",
								Start: ast.Position{
									Column: 45,
									Line:   58,
								},
							},
						},
						Name: "polygon",
					},
					Value: &ast.ObjectExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 55,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "{}",
								Start: ast.Position{
									Column: 53,
									Line:   58,
								},
							},
						},
						Properties: nil,
						With:       nil,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 67,
								Line:   58,
							},
							File:   "geo.flux",
							Source: "minSize=24",
							Start: ast.Position{
								Column: 57,
								Line:   58,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 64,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "minSize",
								Start: ast.Position{
									Column: 57,
									Line:   58,
								},
							},
						},
						Name: "minSize",
					},
					Value: &ast.IntegerLiteral{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 67,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "24",
								Start: ast.Position{
									Column: 65,
									Line:   58,
								},
							},
						},
						Value: int64(24),
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 79,
								Line:   58,
							},
							File:   "geo.flux",
							Source: "maxSize=-1",
							Start: ast.Position{
								Column: 69,
								Line:   58,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 76,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "maxSize",
								Start: ast.Position{
									Column: 69,
									Line:   58,
								},
							},
						},
						Name: "maxSize",
					},
					Value: &ast.UnaryExpression{
						Argument: &ast.IntegerLiteral{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 79,
										Line:   58,
									},
									File:   "geo.flux",
									Source: "1",
									Start: ast.Position{
										Column: 78,
										Line:   58,
									},
								},
							},
							Value: int64(1),
						},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 79,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "-1",
								Start: ast.Position{
									Column: 77,
									Line:   58,
								},
							},
						},
						Operator: 6,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 89,
								Line:   58,
							},
							File:   "geo.flux",
							Source: "level=-1",
							Start: ast.Position{
								Column: 81,
								Line:   58,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 86,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "level",
								Start: ast.Position{
									Column: 81,
									Line:   58,
								},
							},
						},
						Name: "level",
					},
					Value: &ast.UnaryExpression{
						Argument: &ast.IntegerLiteral{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 89,
										Line:   58,
									},
									File:   "geo.flux",
									Source: "1",
									Start: ast.Position{
										Column: 88,
										Line:   58,
									},
								},
							},
							Value: int64(1),
						},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 89,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "-1",
								Start: ast.Position{
									Column: 87,
									Line:   58,
								},
							},
						},
						Operator: 6,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 101,
								Line:   58,
							},
							File:   "geo.flux",
							Source: "ciLevel=-1",
							Start: ast.Position{
								Column: 91,
								Line:   58,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 98,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "ciLevel",
								Start: ast.Position{
									Column: 91,
									Line:   58,
								},
							},
						},
						Name: "ciLevel",
					},
					Value: &ast.UnaryExpression{
						Argument: &ast.IntegerLiteral{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 101,
										Line:   58,
									},
									File:   "geo.flux",
									Source: "1",
									Start: ast.Position{
										Column: 100,
										Line:   58,
									},
								},
							},
							Value: int64(1),
						},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 101,
									Line:   58,
								},
								File:   "geo.flux",
								Source: "-1",
								Start: ast.Position{
									Column: 99,
									Line:   58,
								},
							},
						},
						Operator: 6,
					},
				}},
			},
		}, &ast.VariableAssignment{
			BaseNode: ast.BaseNode{
				Errors: nil,
				Loc: &ast.SourceLocation{
					End: ast.Position{
						Column: 6,
						Line:   82,
					},
					File:   "geo.flux",
					Source: "strictFilter = (tables=<-, box={}, circle={}, polygon={}) =>\n  tables\n    |> filter(fn: (r) =>\n      containsLatLon(box: box, circle: circle, polygon: polygon, lat: r.lat, lon: r.lon)\n    )",
					Start: ast.Position{
						Column: 1,
						Line:   78,
					},
				},
			},
			ID: &ast.Identifier{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 13,
							Line:   78,
						},
						File:   "geo.flux",
						Source: "strictFilter",
						Start: ast.Position{
							Column: 1,
							Line:   78,
						},
					},
				},
				Name: "strictFilter",
			},
			Init: &ast.FunctionExpression{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 6,
							Line:   82,
						},
						File:   "geo.flux",
						Source: "(tables=<-, box={}, circle={}, polygon={}) =>\n  tables\n    |> filter(fn: (r) =>\n      containsLatLon(box: box, circle: circle, polygon: polygon, lat: r.lat, lon: r.lon)\n    )",
						Start: ast.Position{
							Column: 16,
							Line:   78,
						},
					},
				},
				Body: &ast.PipeExpression{
					Argument: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 9,
									Line:   79,
								},
								File:   "geo.flux",
								Source: "tables",
								Start: ast.Position{
									Column: 3,
									Line:   79,
								},
							},
						},
						Name: "tables",
					},
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 6,
								Line:   82,
							},
							File:   "geo.flux",
							Source: "tables\n    |> filter(fn: (r) =>\n      containsLatLon(box: box, circle: circle, polygon: polygon, lat: r.lat, lon: r.lon)\n    )",
							Start: ast.Position{
								Column: 3,
								Line:   79,
							},
						},
					},
					Call: &ast.CallExpression{
						Arguments: []ast.Expression{&ast.ObjectExpression{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 89,
										Line:   81,
									},
									File:   "geo.flux",
									Source: "fn: (r) =>\n      containsLatLon(box: box, circle: circle, polygon: polygon, lat: r.lat, lon: r.lon)",
									Start: ast.Position{
										Column: 15,
										Line:   80,
									},
								},
							},
							Properties: []*ast.Property{&ast.Property{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 89,
											Line:   81,
										},
										File:   "geo.flux",
										Source: "fn: (r) =>\n      containsLatLon(box: box, circle: circle, polygon: polygon, lat: r.lat, lon: r.lon)",
										Start: ast.Position{
											Column: 15,
											Line:   80,
										},
									},
								},
								Key: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 17,
												Line:   80,
											},
											File:   "geo.flux",
											Source: "fn",
											Start: ast.Position{
												Column: 15,
												Line:   80,
											},
										},
									},
									Name: "fn",
								},
								Value: &ast.FunctionExpression{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 89,
												Line:   81,
											},
											File:   "geo.flux",
											Source: "(r) =>\n      containsLatLon(box: box, circle: circle, polygon: polygon, lat: r.lat, lon: r.lon)",
											Start: ast.Position{
												Column: 19,
												Line:   80,
											},
										},
									},
									Body: &ast.CallExpression{
										Arguments: []ast.Expression{&ast.ObjectExpression{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 88,
														Line:   81,
													},
													File:   "geo.flux",
													Source: "box: box, circle: circle, polygon: polygon, lat: r.lat, lon: r.lon",
													Start: ast.Position{
														Column: 22,
														Line:   81,
													},
												},
											},
											Properties: []*ast.Property{&ast.Property{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 30,
															Line:   81,
														},
														File:   "geo.flux",
														Source: "box: box",
														Start: ast.Position{
															Column: 22,
															Line:   81,
														},
													},
												},
												Key: &ast.Identifier{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 25,
																Line:   81,
															},
															File:   "geo.flux",
															Source: "box",
															Start: ast.Position{
																Column: 22,
																Line:   81,
															},
														},
													},
													Name: "box",
												},
												Value: &ast.Identifier{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 30,
																Line:   81,
															},
															File:   "geo.flux",
															Source: "box",
															Start: ast.Position{
																Column: 27,
																Line:   81,
															},
														},
													},
													Name: "box",
												},
											}, &ast.Property{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 46,
															Line:   81,
														},
														File:   "geo.flux",
														Source: "circle: circle",
														Start: ast.Position{
															Column: 32,
															Line:   81,
														},
													},
												},
												Key: &ast.Identifier{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 38,
																Line:   81,
															},
															File:   "geo.flux",
															Source: "circle",
															Start: ast.Position{
																Column: 32,
																Line:   81,
															},
														},
													},
													Name: "circle",
												},
												Value: &ast.Identifier{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 46,
																Line:   81,
															},
															File:   "geo.flux",
															Source: "circle",
															Start: ast.Position{
																Column: 40,
																Line:   81,
															},
														},
													},
													Name: "circle",
												},
											}, &ast.Property{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 64,
															Line:   81,
														},
														File:   "geo.flux",
														Source: "polygon: polygon",
														Start: ast.Position{
															Column: 48,
															Line:   81,
														},
													},
												},
												Key: &ast.Identifier{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 55,
																Line:   81,
															},
															File:   "geo.flux",
															Source: "polygon",
															Start: ast.Position{
																Column: 48,
																Line:   81,
															},
														},
													},
													Name: "polygon",
												},
												Value: &ast.Identifier{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 64,
																Line:   81,
															},
															File:   "geo.flux",
															Source: "polygon",
															Start: ast.Position{
																Column: 57,
																Line:   81,
															},
														},
													},
													Name: "polygon",
												},
											}, &ast.Property{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 76,
															Line:   81,
														},
														File:   "geo.flux",
														Source: "lat: r.lat",
														Start: ast.Position{
															Column: 66,
															Line:   81,
														},
													},
												},
												Key: &ast.Identifier{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 69,
																Line:   81,
															},
															File:   "geo.flux",
															Source: "lat",
															Start: ast.Position{
																Column: 66,
																Line:   81,
															},
														},
													},
													Name: "lat",
												},
												Value: &ast.MemberExpression{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 76,
																Line:   81,
															},
															File:   "geo.flux",
															Source: "r.lat",
															Start: ast.Position{
																Column: 71,
																Line:   81,
															},
														},
													},
													Object: &ast.Identifier{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 72,
																	Line:   81,
																},
																File:   "geo.flux",
																Source: "r",
																Start: ast.Position{
																	Column: 71,
																	Line:   81,
																},
															},
														},
														Name: "r",
													},
													Property: &ast.Identifier{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 76,
																	Line:   81,
																},
																File:   "geo.flux",
																Source: "lat",
																Start: ast.Position{
																	Column: 73,
																	Line:   81,
																},
															},
														},
														Name: "lat",
													},
												},
											}, &ast.Property{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 88,
															Line:   81,
														},
														File:   "geo.flux",
														Source: "lon: r.lon",
														Start: ast.Position{
															Column: 78,
															Line:   81,
														},
													},
												},
												Key: &ast.Identifier{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 81,
																Line:   81,
															},
															File:   "geo.flux",
															Source: "lon",
															Start: ast.Position{
																Column: 78,
																Line:   81,
															},
														},
													},
													Name: "lon",
												},
												Value: &ast.MemberExpression{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 88,
																Line:   81,
															},
															File:   "geo.flux",
															Source: "r.lon",
															Start: ast.Position{
																Column: 83,
																Line:   81,
															},
														},
													},
													Object: &ast.Identifier{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 84,
																	Line:   81,
																},
																File:   "geo.flux",
																Source: "r",
																Start: ast.Position{
																	Column: 83,
																	Line:   81,
																},
															},
														},
														Name: "r",
													},
													Property: &ast.Identifier{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 88,
																	Line:   81,
																},
																File:   "geo.flux",
																Source: "lon",
																Start: ast.Position{
																	Column: 85,
																	Line:   81,
																},
															},
														},
														Name: "lon",
													},
												},
											}},
											With: nil,
										}},
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 89,
													Line:   81,
												},
												File:   "geo.flux",
												Source: "containsLatLon(box: box, circle: circle, polygon: polygon, lat: r.lat, lon: r.lon)",
												Start: ast.Position{
													Column: 7,
													Line:   81,
												},
											},
										},
										Callee: &ast.Identifier{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 21,
														Line:   81,
													},
													File:   "geo.flux",
													Source: "containsLatLon",
													Start: ast.Position{
														Column: 7,
														Line:   81,
													},
												},
											},
											Name: "containsLatLon",
										},
									},
									Params: []*ast.Property{&ast.Property{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 21,
													Line:   80,
												},
												File:   "geo.flux",
												Source: "r",
												Start: ast.Position{
													Column: 20,
													Line:   80,
												},
											},
										},
										Key: &ast.Identifier{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 21,
														Line:   80,
													},
													File:   "geo.flux",
													Source: "r",
													Start: ast.Position{
														Column: 20,
														Line:   80,
													},
												},
											},
											Name: "r",
										},
										Value: nil,
									}},
								},
							}},
							With: nil,
						}},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 6,
									Line:   82,
								},
								File:   "geo.flux",
								Source: "filter(fn: (r) =>\n      containsLatLon(box: box, circle: circle, polygon: polygon, lat: r.lat, lon: r.lon)\n    )",
								Start: ast.Position{
									Column: 8,
									Line:   80,
								},
							},
						},
						Callee: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 14,
										Line:   80,
									},
									File:   "geo.flux",
									Source: "filter",
									Start: ast.Position{
										Column: 8,
										Line:   80,
									},
								},
							},
							Name: "filter",
						},
					},
				},
				Params: []*ast.Property{&ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 26,
								Line:   78,
							},
							File:   "geo.flux",
							Source: "tables=<-",
							Start: ast.Position{
								Column: 17,
								Line:   78,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 23,
									Line:   78,
								},
								File:   "geo.flux",
								Source: "tables",
								Start: ast.Position{
									Column: 17,
									Line:   78,
								},
							},
						},
						Name: "tables",
					},
					Value: &ast.PipeLiteral{BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 26,
								Line:   78,
							},
							File:   "geo.flux",
							Source: "<-",
							Start: ast.Position{
								Column: 24,
								Line:   78,
							},
						},
					}},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 34,
								Line:   78,
							},
							File:   "geo.flux",
							Source: "box={}",
							Start: ast.Position{
								Column: 28,
								Line:   78,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 31,
									Line:   78,
								},
								File:   "geo.flux",
								Source: "box",
								Start: ast.Position{
									Column: 28,
									Line:   78,
								},
							},
						},
						Name: "box",
					},
					Value: &ast.ObjectExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 34,
									Line:   78,
								},
								File:   "geo.flux",
								Source: "{}",
								Start: ast.Position{
									Column: 32,
									Line:   78,
								},
							},
						},
						Properties: nil,
						With:       nil,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 45,
								Line:   78,
							},
							File:   "geo.flux",
							Source: "circle={}",
							Start: ast.Position{
								Column: 36,
								Line:   78,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 42,
									Line:   78,
								},
								File:   "geo.flux",
								Source: "circle",
								Start: ast.Position{
									Column: 36,
									Line:   78,
								},
							},
						},
						Name: "circle",
					},
					Value: &ast.ObjectExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 45,
									Line:   78,
								},
								File:   "geo.flux",
								Source: "{}",
								Start: ast.Position{
									Column: 43,
									Line:   78,
								},
							},
						},
						Properties: nil,
						With:       nil,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 57,
								Line:   78,
							},
							File:   "geo.flux",
							Source: "polygon={}",
							Start: ast.Position{
								Column: 47,
								Line:   78,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 54,
									Line:   78,
								},
								File:   "geo.flux",
								Source: "polygon",
								Start: ast.Position{
									Column: 47,
									Line:   78,
								},
							},
						},
						Name: "polygon",
					},
					Value: &ast.ObjectExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 57,
									Line:   78,
								},
								File:   "geo.flux",
								Source: "{}",
								Start: ast.Position{
									Column: 55,
									Line:   78,
								},
							},
						},
						Properties: nil,
						With:       nil,
					},
				}},
			},
		}, &ast.VariableAssignment{
			BaseNode: ast.BaseNode{
				Errors: nil,
				Loc: &ast.SourceLocation{
					End: ast.Position{
						Column: 2,
						Line:   98,
					},
					File:   "geo.flux",
					Source: "filterRows = (tables=<-, box={}, circle={}, polygon={}, minSize=24, maxSize=-1, level=-1, ciLevel=-1, correlationKey=[\"_time\"], strict=true) => {\n  _rows =\n    tables\n      |> gridFilter(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, ciLevel: ciLevel)\n      |> toRows(correlationKey)\n  _result =\n    if strict then\n      _rows\n        |> strictFilter(box, circle, polygon)\n    else\n      _rows\n  return _result\n}",
					Start: ast.Position{
						Column: 1,
						Line:   86,
					},
				},
			},
			ID: &ast.Identifier{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 11,
							Line:   86,
						},
						File:   "geo.flux",
						Source: "filterRows",
						Start: ast.Position{
							Column: 1,
							Line:   86,
						},
					},
				},
				Name: "filterRows",
			},
			Init: &ast.FunctionExpression{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 2,
							Line:   98,
						},
						File:   "geo.flux",
						Source: "(tables=<-, box={}, circle={}, polygon={}, minSize=24, maxSize=-1, level=-1, ciLevel=-1, correlationKey=[\"_time\"], strict=true) => {\n  _rows =\n    tables\n      |> gridFilter(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, ciLevel: ciLevel)\n      |> toRows(correlationKey)\n  _result =\n    if strict then\n      _rows\n        |> strictFilter(box, circle, polygon)\n    else\n      _rows\n  return _result\n}",
						Start: ast.Position{
							Column: 14,
							Line:   86,
						},
					},
				},
				Body: &ast.Block{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 2,
								Line:   98,
							},
							File:   "geo.flux",
							Source: "{\n  _rows =\n    tables\n      |> gridFilter(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, ciLevel: ciLevel)\n      |> toRows(correlationKey)\n  _result =\n    if strict then\n      _rows\n        |> strictFilter(box, circle, polygon)\n    else\n      _rows\n  return _result\n}",
							Start: ast.Position{
								Column: 145,
								Line:   86,
							},
						},
					},
					Body: []ast.Statement{&ast.VariableAssignment{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 32,
									Line:   90,
								},
								File:   "geo.flux",
								Source: "_rows =\n    tables\n      |> gridFilter(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, ciLevel: ciLevel)\n      |> toRows(correlationKey)",
								Start: ast.Position{
									Column: 3,
									Line:   87,
								},
							},
						},
						ID: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 8,
										Line:   87,
									},
									File:   "geo.flux",
									Source: "_rows",
									Start: ast.Position{
										Column: 3,
										Line:   87,
									},
								},
							},
							Name: "_rows",
						},
						Init: &ast.PipeExpression{
							Argument: &ast.PipeExpression{
								Argument: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 11,
												Line:   88,
											},
											File:   "geo.flux",
											Source: "tables",
											Start: ast.Position{
												Column: 5,
												Line:   88,
											},
										},
									},
									Name: "tables",
								},
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 132,
											Line:   89,
										},
										File:   "geo.flux",
										Source: "tables\n      |> gridFilter(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, ciLevel: ciLevel)",
										Start: ast.Position{
											Column: 5,
											Line:   88,
										},
									},
								},
								Call: &ast.CallExpression{
									Arguments: []ast.Expression{&ast.ObjectExpression{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 131,
													Line:   89,
												},
												File:   "geo.flux",
												Source: "box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, ciLevel: ciLevel",
												Start: ast.Position{
													Column: 21,
													Line:   89,
												},
											},
										},
										Properties: []*ast.Property{&ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 29,
														Line:   89,
													},
													File:   "geo.flux",
													Source: "box: box",
													Start: ast.Position{
														Column: 21,
														Line:   89,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 24,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "box",
														Start: ast.Position{
															Column: 21,
															Line:   89,
														},
													},
												},
												Name: "box",
											},
											Value: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 29,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "box",
														Start: ast.Position{
															Column: 26,
															Line:   89,
														},
													},
												},
												Name: "box",
											},
										}, &ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 45,
														Line:   89,
													},
													File:   "geo.flux",
													Source: "circle: circle",
													Start: ast.Position{
														Column: 31,
														Line:   89,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 37,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "circle",
														Start: ast.Position{
															Column: 31,
															Line:   89,
														},
													},
												},
												Name: "circle",
											},
											Value: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 45,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "circle",
														Start: ast.Position{
															Column: 39,
															Line:   89,
														},
													},
												},
												Name: "circle",
											},
										}, &ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 63,
														Line:   89,
													},
													File:   "geo.flux",
													Source: "polygon: polygon",
													Start: ast.Position{
														Column: 47,
														Line:   89,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 54,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "polygon",
														Start: ast.Position{
															Column: 47,
															Line:   89,
														},
													},
												},
												Name: "polygon",
											},
											Value: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 63,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "polygon",
														Start: ast.Position{
															Column: 56,
															Line:   89,
														},
													},
												},
												Name: "polygon",
											},
										}, &ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 81,
														Line:   89,
													},
													File:   "geo.flux",
													Source: "minSize: minSize",
													Start: ast.Position{
														Column: 65,
														Line:   89,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 72,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "minSize",
														Start: ast.Position{
															Column: 65,
															Line:   89,
														},
													},
												},
												Name: "minSize",
											},
											Value: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 81,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "minSize",
														Start: ast.Position{
															Column: 74,
															Line:   89,
														},
													},
												},
												Name: "minSize",
											},
										}, &ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 99,
														Line:   89,
													},
													File:   "geo.flux",
													Source: "maxSize: maxSize",
													Start: ast.Position{
														Column: 83,
														Line:   89,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 90,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "maxSize",
														Start: ast.Position{
															Column: 83,
															Line:   89,
														},
													},
												},
												Name: "maxSize",
											},
											Value: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 99,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "maxSize",
														Start: ast.Position{
															Column: 92,
															Line:   89,
														},
													},
												},
												Name: "maxSize",
											},
										}, &ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 113,
														Line:   89,
													},
													File:   "geo.flux",
													Source: "level: level",
													Start: ast.Position{
														Column: 101,
														Line:   89,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 106,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "level",
														Start: ast.Position{
															Column: 101,
															Line:   89,
														},
													},
												},
												Name: "level",
											},
											Value: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 113,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "level",
														Start: ast.Position{
															Column: 108,
															Line:   89,
														},
													},
												},
												Name: "level",
											},
										}, &ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 131,
														Line:   89,
													},
													File:   "geo.flux",
													Source: "ciLevel: ciLevel",
													Start: ast.Position{
														Column: 115,
														Line:   89,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 122,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "ciLevel",
														Start: ast.Position{
															Column: 115,
															Line:   89,
														},
													},
												},
												Name: "ciLevel",
											},
											Value: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 131,
															Line:   89,
														},
														File:   "geo.flux",
														Source: "ciLevel",
														Start: ast.Position{
															Column: 124,
															Line:   89,
														},
													},
												},
												Name: "ciLevel",
											},
										}},
										With: nil,
									}},
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 132,
												Line:   89,
											},
											File:   "geo.flux",
											Source: "gridFilter(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, ciLevel: ciLevel)",
											Start: ast.Position{
												Column: 10,
												Line:   89,
											},
										},
									},
									Callee: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 20,
													Line:   89,
												},
												File:   "geo.flux",
												Source: "gridFilter",
												Start: ast.Position{
													Column: 10,
													Line:   89,
												},
											},
										},
										Name: "gridFilter",
									},
								},
							},
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 32,
										Line:   90,
									},
									File:   "geo.flux",
									Source: "tables\n      |> gridFilter(box: box, circle: circle, polygon: polygon, minSize: minSize, maxSize: maxSize, level: level, ciLevel: ciLevel)\n      |> toRows(correlationKey)",
									Start: ast.Position{
										Column: 5,
										Line:   88,
									},
								},
							},
							Call: &ast.CallExpression{
								Arguments: []ast.Expression{&ast.ObjectExpression{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 31,
												Line:   90,
											},
											File:   "geo.flux",
											Source: "correlationKey",
											Start: ast.Position{
												Column: 17,
												Line:   90,
											},
										},
									},
									Properties: []*ast.Property{&ast.Property{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 31,
													Line:   90,
												},
												File:   "geo.flux",
												Source: "correlationKey",
												Start: ast.Position{
													Column: 17,
													Line:   90,
												},
											},
										},
										Key: &ast.Identifier{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 31,
														Line:   90,
													},
													File:   "geo.flux",
													Source: "correlationKey",
													Start: ast.Position{
														Column: 17,
														Line:   90,
													},
												},
											},
											Name: "correlationKey",
										},
										Value: nil,
									}},
									With: nil,
								}},
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 32,
											Line:   90,
										},
										File:   "geo.flux",
										Source: "toRows(correlationKey)",
										Start: ast.Position{
											Column: 10,
											Line:   90,
										},
									},
								},
								Callee: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 16,
												Line:   90,
											},
											File:   "geo.flux",
											Source: "toRows",
											Start: ast.Position{
												Column: 10,
												Line:   90,
											},
										},
									},
									Name: "toRows",
								},
							},
						},
					}, &ast.VariableAssignment{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 12,
									Line:   96,
								},
								File:   "geo.flux",
								Source: "_result =\n    if strict then\n      _rows\n        |> strictFilter(box, circle, polygon)\n    else\n      _rows",
								Start: ast.Position{
									Column: 3,
									Line:   91,
								},
							},
						},
						ID: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 10,
										Line:   91,
									},
									File:   "geo.flux",
									Source: "_result",
									Start: ast.Position{
										Column: 3,
										Line:   91,
									},
								},
							},
							Name: "_result",
						},
						Init: &ast.ConditionalExpression{
							Alternate: &ast.Identifier{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 12,
											Line:   96,
										},
										File:   "geo.flux",
										Source: "_rows",
										Start: ast.Position{
											Column: 7,
											Line:   96,
										},
									},
								},
								Name: "_rows",
							},
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 12,
										Line:   96,
									},
									File:   "geo.flux",
									Source: "if strict then\n      _rows\n        |> strictFilter(box, circle, polygon)\n    else\n      _rows",
									Start: ast.Position{
										Column: 5,
										Line:   92,
									},
								},
							},
							Consequent: &ast.PipeExpression{
								Argument: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 12,
												Line:   93,
											},
											File:   "geo.flux",
											Source: "_rows",
											Start: ast.Position{
												Column: 7,
												Line:   93,
											},
										},
									},
									Name: "_rows",
								},
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 46,
											Line:   94,
										},
										File:   "geo.flux",
										Source: "_rows\n        |> strictFilter(box, circle, polygon)",
										Start: ast.Position{
											Column: 7,
											Line:   93,
										},
									},
								},
								Call: &ast.CallExpression{
									Arguments: []ast.Expression{&ast.ObjectExpression{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 45,
													Line:   94,
												},
												File:   "geo.flux",
												Source: "box, circle, polygon",
												Start: ast.Position{
													Column: 25,
													Line:   94,
												},
											},
										},
										Properties: []*ast.Property{&ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 28,
														Line:   94,
													},
													File:   "geo.flux",
													Source: "box",
													Start: ast.Position{
														Column: 25,
														Line:   94,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 28,
															Line:   94,
														},
														File:   "geo.flux",
														Source: "box",
														Start: ast.Position{
															Column: 25,
															Line:   94,
														},
													},
												},
												Name: "box",
											},
											Value: nil,
										}, &ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 36,
														Line:   94,
													},
													File:   "geo.flux",
													Source: "circle",
													Start: ast.Position{
														Column: 30,
														Line:   94,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 36,
															Line:   94,
														},
														File:   "geo.flux",
														Source: "circle",
														Start: ast.Position{
															Column: 30,
															Line:   94,
														},
													},
												},
												Name: "circle",
											},
											Value: nil,
										}, &ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 45,
														Line:   94,
													},
													File:   "geo.flux",
													Source: "polygon",
													Start: ast.Position{
														Column: 38,
														Line:   94,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 45,
															Line:   94,
														},
														File:   "geo.flux",
														Source: "polygon",
														Start: ast.Position{
															Column: 38,
															Line:   94,
														},
													},
												},
												Name: "polygon",
											},
											Value: nil,
										}},
										With: nil,
									}},
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 46,
												Line:   94,
											},
											File:   "geo.flux",
											Source: "strictFilter(box, circle, polygon)",
											Start: ast.Position{
												Column: 12,
												Line:   94,
											},
										},
									},
									Callee: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 24,
													Line:   94,
												},
												File:   "geo.flux",
												Source: "strictFilter",
												Start: ast.Position{
													Column: 12,
													Line:   94,
												},
											},
										},
										Name: "strictFilter",
									},
								},
							},
							Test: &ast.Identifier{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 14,
											Line:   92,
										},
										File:   "geo.flux",
										Source: "strict",
										Start: ast.Position{
											Column: 8,
											Line:   92,
										},
									},
								},
								Name: "strict",
							},
						},
					}, &ast.ReturnStatement{
						Argument: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 17,
										Line:   97,
									},
									File:   "geo.flux",
									Source: "_result",
									Start: ast.Position{
										Column: 10,
										Line:   97,
									},
								},
							},
							Name: "_result",
						},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 17,
									Line:   97,
								},
								File:   "geo.flux",
								Source: "return _result",
								Start: ast.Position{
									Column: 3,
									Line:   97,
								},
							},
						},
					}},
				},
				Params: []*ast.Property{&ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 24,
								Line:   86,
							},
							File:   "geo.flux",
							Source: "tables=<-",
							Start: ast.Position{
								Column: 15,
								Line:   86,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 21,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "tables",
								Start: ast.Position{
									Column: 15,
									Line:   86,
								},
							},
						},
						Name: "tables",
					},
					Value: &ast.PipeLiteral{BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 24,
								Line:   86,
							},
							File:   "geo.flux",
							Source: "<-",
							Start: ast.Position{
								Column: 22,
								Line:   86,
							},
						},
					}},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 32,
								Line:   86,
							},
							File:   "geo.flux",
							Source: "box={}",
							Start: ast.Position{
								Column: 26,
								Line:   86,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 29,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "box",
								Start: ast.Position{
									Column: 26,
									Line:   86,
								},
							},
						},
						Name: "box",
					},
					Value: &ast.ObjectExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 32,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "{}",
								Start: ast.Position{
									Column: 30,
									Line:   86,
								},
							},
						},
						Properties: nil,
						With:       nil,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 43,
								Line:   86,
							},
							File:   "geo.flux",
							Source: "circle={}",
							Start: ast.Position{
								Column: 34,
								Line:   86,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 40,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "circle",
								Start: ast.Position{
									Column: 34,
									Line:   86,
								},
							},
						},
						Name: "circle",
					},
					Value: &ast.ObjectExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 43,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "{}",
								Start: ast.Position{
									Column: 41,
									Line:   86,
								},
							},
						},
						Properties: nil,
						With:       nil,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 55,
								Line:   86,
							},
							File:   "geo.flux",
							Source: "polygon={}",
							Start: ast.Position{
								Column: 45,
								Line:   86,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 52,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "polygon",
								Start: ast.Position{
									Column: 45,
									Line:   86,
								},
							},
						},
						Name: "polygon",
					},
					Value: &ast.ObjectExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 55,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "{}",
								Start: ast.Position{
									Column: 53,
									Line:   86,
								},
							},
						},
						Properties: nil,
						With:       nil,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 67,
								Line:   86,
							},
							File:   "geo.flux",
							Source: "minSize=24",
							Start: ast.Position{
								Column: 57,
								Line:   86,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 64,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "minSize",
								Start: ast.Position{
									Column: 57,
									Line:   86,
								},
							},
						},
						Name: "minSize",
					},
					Value: &ast.IntegerLiteral{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 67,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "24",
								Start: ast.Position{
									Column: 65,
									Line:   86,
								},
							},
						},
						Value: int64(24),
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 79,
								Line:   86,
							},
							File:   "geo.flux",
							Source: "maxSize=-1",
							Start: ast.Position{
								Column: 69,
								Line:   86,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 76,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "maxSize",
								Start: ast.Position{
									Column: 69,
									Line:   86,
								},
							},
						},
						Name: "maxSize",
					},
					Value: &ast.UnaryExpression{
						Argument: &ast.IntegerLiteral{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 79,
										Line:   86,
									},
									File:   "geo.flux",
									Source: "1",
									Start: ast.Position{
										Column: 78,
										Line:   86,
									},
								},
							},
							Value: int64(1),
						},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 79,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "-1",
								Start: ast.Position{
									Column: 77,
									Line:   86,
								},
							},
						},
						Operator: 6,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 89,
								Line:   86,
							},
							File:   "geo.flux",
							Source: "level=-1",
							Start: ast.Position{
								Column: 81,
								Line:   86,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 86,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "level",
								Start: ast.Position{
									Column: 81,
									Line:   86,
								},
							},
						},
						Name: "level",
					},
					Value: &ast.UnaryExpression{
						Argument: &ast.IntegerLiteral{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 89,
										Line:   86,
									},
									File:   "geo.flux",
									Source: "1",
									Start: ast.Position{
										Column: 88,
										Line:   86,
									},
								},
							},
							Value: int64(1),
						},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 89,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "-1",
								Start: ast.Position{
									Column: 87,
									Line:   86,
								},
							},
						},
						Operator: 6,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 101,
								Line:   86,
							},
							File:   "geo.flux",
							Source: "ciLevel=-1",
							Start: ast.Position{
								Column: 91,
								Line:   86,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 98,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "ciLevel",
								Start: ast.Position{
									Column: 91,
									Line:   86,
								},
							},
						},
						Name: "ciLevel",
					},
					Value: &ast.UnaryExpression{
						Argument: &ast.IntegerLiteral{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 101,
										Line:   86,
									},
									File:   "geo.flux",
									Source: "1",
									Start: ast.Position{
										Column: 100,
										Line:   86,
									},
								},
							},
							Value: int64(1),
						},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 101,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "-1",
								Start: ast.Position{
									Column: 99,
									Line:   86,
								},
							},
						},
						Operator: 6,
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 127,
								Line:   86,
							},
							File:   "geo.flux",
							Source: "correlationKey=[\"_time\"]",
							Start: ast.Position{
								Column: 103,
								Line:   86,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 117,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "correlationKey",
								Start: ast.Position{
									Column: 103,
									Line:   86,
								},
							},
						},
						Name: "correlationKey",
					},
					Value: &ast.ArrayExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 127,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "[\"_time\"]",
								Start: ast.Position{
									Column: 118,
									Line:   86,
								},
							},
						},
						Elements: []ast.Expression{&ast.StringLiteral{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 126,
										Line:   86,
									},
									File:   "geo.flux",
									Source: "\"_time\"",
									Start: ast.Position{
										Column: 119,
										Line:   86,
									},
								},
							},
							Value: "_time",
						}},
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 140,
								Line:   86,
							},
							File:   "geo.flux",
							Source: "strict=true",
							Start: ast.Position{
								Column: 129,
								Line:   86,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 135,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "strict",
								Start: ast.Position{
									Column: 129,
									Line:   86,
								},
							},
						},
						Name: "strict",
					},
					Value: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 140,
									Line:   86,
								},
								File:   "geo.flux",
								Source: "true",
								Start: ast.Position{
									Column: 136,
									Line:   86,
								},
							},
						},
						Name: "true",
					},
				}},
			},
		}, &ast.VariableAssignment{
			BaseNode: ast.BaseNode{
				Errors: nil,
				Loc: &ast.SourceLocation{
					End: ast.Position{
						Column: 2,
						Line:   118,
					},
					File:   "geo.flux",
					Source: "groupByArea = (tables=<-, newColumn, level, ciLevel=-1) => {\n  _prepared =\n    if level == ciLevel then\n      tables\n\t    |> duplicate(column: \"_ci\", as: newColumn)\n    else\n      tables\n        |> map(fn: (r) => ({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) }))\n        |> rename(columns: { _cixxx: newColumn })\n  return _prepared\n    |> group(columns: [newColumn])\n}",
					Start: ast.Position{
						Column: 1,
						Line:   107,
					},
				},
			},
			ID: &ast.Identifier{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 12,
							Line:   107,
						},
						File:   "geo.flux",
						Source: "groupByArea",
						Start: ast.Position{
							Column: 1,
							Line:   107,
						},
					},
				},
				Name: "groupByArea",
			},
			Init: &ast.FunctionExpression{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 2,
							Line:   118,
						},
						File:   "geo.flux",
						Source: "(tables=<-, newColumn, level, ciLevel=-1) => {\n  _prepared =\n    if level == ciLevel then\n      tables\n\t    |> duplicate(column: \"_ci\", as: newColumn)\n    else\n      tables\n        |> map(fn: (r) => ({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) }))\n        |> rename(columns: { _cixxx: newColumn })\n  return _prepared\n    |> group(columns: [newColumn])\n}",
						Start: ast.Position{
							Column: 15,
							Line:   107,
						},
					},
				},
				Body: &ast.Block{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 2,
								Line:   118,
							},
							File:   "geo.flux",
							Source: "{\n  _prepared =\n    if level == ciLevel then\n      tables\n\t    |> duplicate(column: \"_ci\", as: newColumn)\n    else\n      tables\n        |> map(fn: (r) => ({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) }))\n        |> rename(columns: { _cixxx: newColumn })\n  return _prepared\n    |> group(columns: [newColumn])\n}",
							Start: ast.Position{
								Column: 60,
								Line:   107,
							},
						},
					},
					Body: []ast.Statement{&ast.VariableAssignment{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 50,
									Line:   115,
								},
								File:   "geo.flux",
								Source: "_prepared =\n    if level == ciLevel then\n      tables\n\t    |> duplicate(column: \"_ci\", as: newColumn)\n    else\n      tables\n        |> map(fn: (r) => ({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) }))\n        |> rename(columns: { _cixxx: newColumn })",
								Start: ast.Position{
									Column: 3,
									Line:   108,
								},
							},
						},
						ID: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 12,
										Line:   108,
									},
									File:   "geo.flux",
									Source: "_prepared",
									Start: ast.Position{
										Column: 3,
										Line:   108,
									},
								},
							},
							Name: "_prepared",
						},
						Init: &ast.ConditionalExpression{
							Alternate: &ast.PipeExpression{
								Argument: &ast.PipeExpression{
									Argument: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 13,
													Line:   113,
												},
												File:   "geo.flux",
												Source: "tables",
												Start: ast.Position{
													Column: 7,
													Line:   113,
												},
											},
										},
										Name: "tables",
									},
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 105,
												Line:   114,
											},
											File:   "geo.flux",
											Source: "tables\n        |> map(fn: (r) => ({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) }))",
											Start: ast.Position{
												Column: 7,
												Line:   113,
											},
										},
									},
									Call: &ast.CallExpression{
										Arguments: []ast.Expression{&ast.ObjectExpression{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 104,
														Line:   114,
													},
													File:   "geo.flux",
													Source: "fn: (r) => ({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) })",
													Start: ast.Position{
														Column: 16,
														Line:   114,
													},
												},
											},
											Properties: []*ast.Property{&ast.Property{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 104,
															Line:   114,
														},
														File:   "geo.flux",
														Source: "fn: (r) => ({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) })",
														Start: ast.Position{
															Column: 16,
															Line:   114,
														},
													},
												},
												Key: &ast.Identifier{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 18,
																Line:   114,
															},
															File:   "geo.flux",
															Source: "fn",
															Start: ast.Position{
																Column: 16,
																Line:   114,
															},
														},
													},
													Name: "fn",
												},
												Value: &ast.FunctionExpression{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 104,
																Line:   114,
															},
															File:   "geo.flux",
															Source: "(r) => ({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) })",
															Start: ast.Position{
																Column: 20,
																Line:   114,
															},
														},
													},
													Body: &ast.ParenExpression{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 104,
																	Line:   114,
																},
																File:   "geo.flux",
																Source: "({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) })",
																Start: ast.Position{
																	Column: 27,
																	Line:   114,
																},
															},
														},
														Expression: &ast.ObjectExpression{
															BaseNode: ast.BaseNode{
																Errors: nil,
																Loc: &ast.SourceLocation{
																	End: ast.Position{
																		Column: 103,
																		Line:   114,
																	},
																	File:   "geo.flux",
																	Source: "{ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) }",
																	Start: ast.Position{
																		Column: 28,
																		Line:   114,
																	},
																},
															},
															Properties: []*ast.Property{&ast.Property{
																BaseNode: ast.BaseNode{
																	Errors: nil,
																	Loc: &ast.SourceLocation{
																		End: ast.Position{
																			Column: 101,
																			Line:   114,
																		},
																		File:   "geo.flux",
																		Source: "_cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level)",
																		Start: ast.Position{
																			Column: 37,
																			Line:   114,
																		},
																	},
																},
																Key: &ast.Identifier{
																	BaseNode: ast.BaseNode{
																		Errors: nil,
																		Loc: &ast.SourceLocation{
																			End: ast.Position{
																				Column: 43,
																				Line:   114,
																			},
																			File:   "geo.flux",
																			Source: "_cixxx",
																			Start: ast.Position{
																				Column: 37,
																				Line:   114,
																			},
																		},
																	},
																	Name: "_cixxx",
																},
																Value: &ast.CallExpression{
																	Arguments: []ast.Expression{&ast.ObjectExpression{
																		BaseNode: ast.BaseNode{
																			Errors: nil,
																			Loc: &ast.SourceLocation{
																				End: ast.Position{
																					Column: 100,
																					Line:   114,
																				},
																				File:   "geo.flux",
																				Source: "point: {lat: r.lat, lon: r.lon}, level: level",
																				Start: ast.Position{
																					Column: 55,
																					Line:   114,
																				},
																			},
																		},
																		Properties: []*ast.Property{&ast.Property{
																			BaseNode: ast.BaseNode{
																				Errors: nil,
																				Loc: &ast.SourceLocation{
																					End: ast.Position{
																						Column: 86,
																						Line:   114,
																					},
																					File:   "geo.flux",
																					Source: "point: {lat: r.lat, lon: r.lon}",
																					Start: ast.Position{
																						Column: 55,
																						Line:   114,
																					},
																				},
																			},
																			Key: &ast.Identifier{
																				BaseNode: ast.BaseNode{
																					Errors: nil,
																					Loc: &ast.SourceLocation{
																						End: ast.Position{
																							Column: 60,
																							Line:   114,
																						},
																						File:   "geo.flux",
																						Source: "point",
																						Start: ast.Position{
																							Column: 55,
																							Line:   114,
																						},
																					},
																				},
																				Name: "point",
																			},
																			Value: &ast.ObjectExpression{
																				BaseNode: ast.BaseNode{
																					Errors: nil,
																					Loc: &ast.SourceLocation{
																						End: ast.Position{
																							Column: 86,
																							Line:   114,
																						},
																						File:   "geo.flux",
																						Source: "{lat: r.lat, lon: r.lon}",
																						Start: ast.Position{
																							Column: 62,
																							Line:   114,
																						},
																					},
																				},
																				Properties: []*ast.Property{&ast.Property{
																					BaseNode: ast.BaseNode{
																						Errors: nil,
																						Loc: &ast.SourceLocation{
																							End: ast.Position{
																								Column: 73,
																								Line:   114,
																							},
																							File:   "geo.flux",
																							Source: "lat: r.lat",
																							Start: ast.Position{
																								Column: 63,
																								Line:   114,
																							},
																						},
																					},
																					Key: &ast.Identifier{
																						BaseNode: ast.BaseNode{
																							Errors: nil,
																							Loc: &ast.SourceLocation{
																								End: ast.Position{
																									Column: 66,
																									Line:   114,
																								},
																								File:   "geo.flux",
																								Source: "lat",
																								Start: ast.Position{
																									Column: 63,
																									Line:   114,
																								},
																							},
																						},
																						Name: "lat",
																					},
																					Value: &ast.MemberExpression{
																						BaseNode: ast.BaseNode{
																							Errors: nil,
																							Loc: &ast.SourceLocation{
																								End: ast.Position{
																									Column: 73,
																									Line:   114,
																								},
																								File:   "geo.flux",
																								Source: "r.lat",
																								Start: ast.Position{
																									Column: 68,
																									Line:   114,
																								},
																							},
																						},
																						Object: &ast.Identifier{
																							BaseNode: ast.BaseNode{
																								Errors: nil,
																								Loc: &ast.SourceLocation{
																									End: ast.Position{
																										Column: 69,
																										Line:   114,
																									},
																									File:   "geo.flux",
																									Source: "r",
																									Start: ast.Position{
																										Column: 68,
																										Line:   114,
																									},
																								},
																							},
																							Name: "r",
																						},
																						Property: &ast.Identifier{
																							BaseNode: ast.BaseNode{
																								Errors: nil,
																								Loc: &ast.SourceLocation{
																									End: ast.Position{
																										Column: 73,
																										Line:   114,
																									},
																									File:   "geo.flux",
																									Source: "lat",
																									Start: ast.Position{
																										Column: 70,
																										Line:   114,
																									},
																								},
																							},
																							Name: "lat",
																						},
																					},
																				}, &ast.Property{
																					BaseNode: ast.BaseNode{
																						Errors: nil,
																						Loc: &ast.SourceLocation{
																							End: ast.Position{
																								Column: 85,
																								Line:   114,
																							},
																							File:   "geo.flux",
																							Source: "lon: r.lon",
																							Start: ast.Position{
																								Column: 75,
																								Line:   114,
																							},
																						},
																					},
																					Key: &ast.Identifier{
																						BaseNode: ast.BaseNode{
																							Errors: nil,
																							Loc: &ast.SourceLocation{
																								End: ast.Position{
																									Column: 78,
																									Line:   114,
																								},
																								File:   "geo.flux",
																								Source: "lon",
																								Start: ast.Position{
																									Column: 75,
																									Line:   114,
																								},
																							},
																						},
																						Name: "lon",
																					},
																					Value: &ast.MemberExpression{
																						BaseNode: ast.BaseNode{
																							Errors: nil,
																							Loc: &ast.SourceLocation{
																								End: ast.Position{
																									Column: 85,
																									Line:   114,
																								},
																								File:   "geo.flux",
																								Source: "r.lon",
																								Start: ast.Position{
																									Column: 80,
																									Line:   114,
																								},
																							},
																						},
																						Object: &ast.Identifier{
																							BaseNode: ast.BaseNode{
																								Errors: nil,
																								Loc: &ast.SourceLocation{
																									End: ast.Position{
																										Column: 81,
																										Line:   114,
																									},
																									File:   "geo.flux",
																									Source: "r",
																									Start: ast.Position{
																										Column: 80,
																										Line:   114,
																									},
																								},
																							},
																							Name: "r",
																						},
																						Property: &ast.Identifier{
																							BaseNode: ast.BaseNode{
																								Errors: nil,
																								Loc: &ast.SourceLocation{
																									End: ast.Position{
																										Column: 85,
																										Line:   114,
																									},
																									File:   "geo.flux",
																									Source: "lon",
																									Start: ast.Position{
																										Column: 82,
																										Line:   114,
																									},
																								},
																							},
																							Name: "lon",
																						},
																					},
																				}},
																				With: nil,
																			},
																		}, &ast.Property{
																			BaseNode: ast.BaseNode{
																				Errors: nil,
																				Loc: &ast.SourceLocation{
																					End: ast.Position{
																						Column: 100,
																						Line:   114,
																					},
																					File:   "geo.flux",
																					Source: "level: level",
																					Start: ast.Position{
																						Column: 88,
																						Line:   114,
																					},
																				},
																			},
																			Key: &ast.Identifier{
																				BaseNode: ast.BaseNode{
																					Errors: nil,
																					Loc: &ast.SourceLocation{
																						End: ast.Position{
																							Column: 93,
																							Line:   114,
																						},
																						File:   "geo.flux",
																						Source: "level",
																						Start: ast.Position{
																							Column: 88,
																							Line:   114,
																						},
																					},
																				},
																				Name: "level",
																			},
																			Value: &ast.Identifier{
																				BaseNode: ast.BaseNode{
																					Errors: nil,
																					Loc: &ast.SourceLocation{
																						End: ast.Position{
																							Column: 100,
																							Line:   114,
																						},
																						File:   "geo.flux",
																						Source: "level",
																						Start: ast.Position{
																							Column: 95,
																							Line:   114,
																						},
																					},
																				},
																				Name: "level",
																			},
																		}},
																		With: nil,
																	}},
																	BaseNode: ast.BaseNode{
																		Errors: nil,
																		Loc: &ast.SourceLocation{
																			End: ast.Position{
																				Column: 101,
																				Line:   114,
																			},
																			File:   "geo.flux",
																			Source: "getParent(point: {lat: r.lat, lon: r.lon}, level: level)",
																			Start: ast.Position{
																				Column: 45,
																				Line:   114,
																			},
																		},
																	},
																	Callee: &ast.Identifier{
																		BaseNode: ast.BaseNode{
																			Errors: nil,
																			Loc: &ast.SourceLocation{
																				End: ast.Position{
																					Column: 54,
																					Line:   114,
																				},
																				File:   "geo.flux",
																				Source: "getParent",
																				Start: ast.Position{
																					Column: 45,
																					Line:   114,
																				},
																			},
																		},
																		Name: "getParent",
																	},
																},
															}},
															With: &ast.Identifier{
																BaseNode: ast.BaseNode{
																	Errors: nil,
																	Loc: &ast.SourceLocation{
																		End: ast.Position{
																			Column: 31,
																			Line:   114,
																		},
																		File:   "geo.flux",
																		Source: "r",
																		Start: ast.Position{
																			Column: 30,
																			Line:   114,
																		},
																	},
																},
																Name: "r",
															},
														},
													},
													Params: []*ast.Property{&ast.Property{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 22,
																	Line:   114,
																},
																File:   "geo.flux",
																Source: "r",
																Start: ast.Position{
																	Column: 21,
																	Line:   114,
																},
															},
														},
														Key: &ast.Identifier{
															BaseNode: ast.BaseNode{
																Errors: nil,
																Loc: &ast.SourceLocation{
																	End: ast.Position{
																		Column: 22,
																		Line:   114,
																	},
																	File:   "geo.flux",
																	Source: "r",
																	Start: ast.Position{
																		Column: 21,
																		Line:   114,
																	},
																},
															},
															Name: "r",
														},
														Value: nil,
													}},
												},
											}},
											With: nil,
										}},
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 105,
													Line:   114,
												},
												File:   "geo.flux",
												Source: "map(fn: (r) => ({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) }))",
												Start: ast.Position{
													Column: 12,
													Line:   114,
												},
											},
										},
										Callee: &ast.Identifier{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 15,
														Line:   114,
													},
													File:   "geo.flux",
													Source: "map",
													Start: ast.Position{
														Column: 12,
														Line:   114,
													},
												},
											},
											Name: "map",
										},
									},
								},
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 50,
											Line:   115,
										},
										File:   "geo.flux",
										Source: "tables\n        |> map(fn: (r) => ({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) }))\n        |> rename(columns: { _cixxx: newColumn })",
										Start: ast.Position{
											Column: 7,
											Line:   113,
										},
									},
								},
								Call: &ast.CallExpression{
									Arguments: []ast.Expression{&ast.ObjectExpression{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 49,
													Line:   115,
												},
												File:   "geo.flux",
												Source: "columns: { _cixxx: newColumn }",
												Start: ast.Position{
													Column: 19,
													Line:   115,
												},
											},
										},
										Properties: []*ast.Property{&ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 49,
														Line:   115,
													},
													File:   "geo.flux",
													Source: "columns: { _cixxx: newColumn }",
													Start: ast.Position{
														Column: 19,
														Line:   115,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 26,
															Line:   115,
														},
														File:   "geo.flux",
														Source: "columns",
														Start: ast.Position{
															Column: 19,
															Line:   115,
														},
													},
												},
												Name: "columns",
											},
											Value: &ast.ObjectExpression{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 49,
															Line:   115,
														},
														File:   "geo.flux",
														Source: "{ _cixxx: newColumn }",
														Start: ast.Position{
															Column: 28,
															Line:   115,
														},
													},
												},
												Properties: []*ast.Property{&ast.Property{
													BaseNode: ast.BaseNode{
														Errors: nil,
														Loc: &ast.SourceLocation{
															End: ast.Position{
																Column: 47,
																Line:   115,
															},
															File:   "geo.flux",
															Source: "_cixxx: newColumn",
															Start: ast.Position{
																Column: 30,
																Line:   115,
															},
														},
													},
													Key: &ast.Identifier{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 36,
																	Line:   115,
																},
																File:   "geo.flux",
																Source: "_cixxx",
																Start: ast.Position{
																	Column: 30,
																	Line:   115,
																},
															},
														},
														Name: "_cixxx",
													},
													Value: &ast.Identifier{
														BaseNode: ast.BaseNode{
															Errors: nil,
															Loc: &ast.SourceLocation{
																End: ast.Position{
																	Column: 47,
																	Line:   115,
																},
																File:   "geo.flux",
																Source: "newColumn",
																Start: ast.Position{
																	Column: 38,
																	Line:   115,
																},
															},
														},
														Name: "newColumn",
													},
												}},
												With: nil,
											},
										}},
										With: nil,
									}},
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 50,
												Line:   115,
											},
											File:   "geo.flux",
											Source: "rename(columns: { _cixxx: newColumn })",
											Start: ast.Position{
												Column: 12,
												Line:   115,
											},
										},
									},
									Callee: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 18,
													Line:   115,
												},
												File:   "geo.flux",
												Source: "rename",
												Start: ast.Position{
													Column: 12,
													Line:   115,
												},
											},
										},
										Name: "rename",
									},
								},
							},
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 50,
										Line:   115,
									},
									File:   "geo.flux",
									Source: "if level == ciLevel then\n      tables\n\t    |> duplicate(column: \"_ci\", as: newColumn)\n    else\n      tables\n        |> map(fn: (r) => ({ r with _cixxx: getParent(point: {lat: r.lat, lon: r.lon}, level: level) }))\n        |> rename(columns: { _cixxx: newColumn })",
									Start: ast.Position{
										Column: 5,
										Line:   109,
									},
								},
							},
							Consequent: &ast.PipeExpression{
								Argument: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 13,
												Line:   110,
											},
											File:   "geo.flux",
											Source: "tables",
											Start: ast.Position{
												Column: 7,
												Line:   110,
											},
										},
									},
									Name: "tables",
								},
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 48,
											Line:   111,
										},
										File:   "geo.flux",
										Source: "tables\n\t    |> duplicate(column: \"_ci\", as: newColumn)",
										Start: ast.Position{
											Column: 7,
											Line:   110,
										},
									},
								},
								Call: &ast.CallExpression{
									Arguments: []ast.Expression{&ast.ObjectExpression{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 47,
													Line:   111,
												},
												File:   "geo.flux",
												Source: "column: \"_ci\", as: newColumn",
												Start: ast.Position{
													Column: 19,
													Line:   111,
												},
											},
										},
										Properties: []*ast.Property{&ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 32,
														Line:   111,
													},
													File:   "geo.flux",
													Source: "column: \"_ci\"",
													Start: ast.Position{
														Column: 19,
														Line:   111,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 25,
															Line:   111,
														},
														File:   "geo.flux",
														Source: "column",
														Start: ast.Position{
															Column: 19,
															Line:   111,
														},
													},
												},
												Name: "column",
											},
											Value: &ast.StringLiteral{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 32,
															Line:   111,
														},
														File:   "geo.flux",
														Source: "\"_ci\"",
														Start: ast.Position{
															Column: 27,
															Line:   111,
														},
													},
												},
												Value: "_ci",
											},
										}, &ast.Property{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 47,
														Line:   111,
													},
													File:   "geo.flux",
													Source: "as: newColumn",
													Start: ast.Position{
														Column: 34,
														Line:   111,
													},
												},
											},
											Key: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 36,
															Line:   111,
														},
														File:   "geo.flux",
														Source: "as",
														Start: ast.Position{
															Column: 34,
															Line:   111,
														},
													},
												},
												Name: "as",
											},
											Value: &ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 47,
															Line:   111,
														},
														File:   "geo.flux",
														Source: "newColumn",
														Start: ast.Position{
															Column: 38,
															Line:   111,
														},
													},
												},
												Name: "newColumn",
											},
										}},
										With: nil,
									}},
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 48,
												Line:   111,
											},
											File:   "geo.flux",
											Source: "duplicate(column: \"_ci\", as: newColumn)",
											Start: ast.Position{
												Column: 9,
												Line:   111,
											},
										},
									},
									Callee: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 18,
													Line:   111,
												},
												File:   "geo.flux",
												Source: "duplicate",
												Start: ast.Position{
													Column: 9,
													Line:   111,
												},
											},
										},
										Name: "duplicate",
									},
								},
							},
							Test: &ast.BinaryExpression{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 24,
											Line:   109,
										},
										File:   "geo.flux",
										Source: "level == ciLevel",
										Start: ast.Position{
											Column: 8,
											Line:   109,
										},
									},
								},
								Left: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 13,
												Line:   109,
											},
											File:   "geo.flux",
											Source: "level",
											Start: ast.Position{
												Column: 8,
												Line:   109,
											},
										},
									},
									Name: "level",
								},
								Operator: 17,
								Right: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 24,
												Line:   109,
											},
											File:   "geo.flux",
											Source: "ciLevel",
											Start: ast.Position{
												Column: 17,
												Line:   109,
											},
										},
									},
									Name: "ciLevel",
								},
							},
						},
					}, &ast.ReturnStatement{
						Argument: &ast.PipeExpression{
							Argument: &ast.Identifier{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 19,
											Line:   116,
										},
										File:   "geo.flux",
										Source: "_prepared",
										Start: ast.Position{
											Column: 10,
											Line:   116,
										},
									},
								},
								Name: "_prepared",
							},
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 35,
										Line:   117,
									},
									File:   "geo.flux",
									Source: "_prepared\n    |> group(columns: [newColumn])",
									Start: ast.Position{
										Column: 10,
										Line:   116,
									},
								},
							},
							Call: &ast.CallExpression{
								Arguments: []ast.Expression{&ast.ObjectExpression{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 34,
												Line:   117,
											},
											File:   "geo.flux",
											Source: "columns: [newColumn]",
											Start: ast.Position{
												Column: 14,
												Line:   117,
											},
										},
									},
									Properties: []*ast.Property{&ast.Property{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 34,
													Line:   117,
												},
												File:   "geo.flux",
												Source: "columns: [newColumn]",
												Start: ast.Position{
													Column: 14,
													Line:   117,
												},
											},
										},
										Key: &ast.Identifier{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 21,
														Line:   117,
													},
													File:   "geo.flux",
													Source: "columns",
													Start: ast.Position{
														Column: 14,
														Line:   117,
													},
												},
											},
											Name: "columns",
										},
										Value: &ast.ArrayExpression{
											BaseNode: ast.BaseNode{
												Errors: nil,
												Loc: &ast.SourceLocation{
													End: ast.Position{
														Column: 34,
														Line:   117,
													},
													File:   "geo.flux",
													Source: "[newColumn]",
													Start: ast.Position{
														Column: 23,
														Line:   117,
													},
												},
											},
											Elements: []ast.Expression{&ast.Identifier{
												BaseNode: ast.BaseNode{
													Errors: nil,
													Loc: &ast.SourceLocation{
														End: ast.Position{
															Column: 33,
															Line:   117,
														},
														File:   "geo.flux",
														Source: "newColumn",
														Start: ast.Position{
															Column: 24,
															Line:   117,
														},
													},
												},
												Name: "newColumn",
											}},
										},
									}},
									With: nil,
								}},
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 35,
											Line:   117,
										},
										File:   "geo.flux",
										Source: "group(columns: [newColumn])",
										Start: ast.Position{
											Column: 8,
											Line:   117,
										},
									},
								},
								Callee: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 13,
												Line:   117,
											},
											File:   "geo.flux",
											Source: "group",
											Start: ast.Position{
												Column: 8,
												Line:   117,
											},
										},
									},
									Name: "group",
								},
							},
						},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 35,
									Line:   117,
								},
								File:   "geo.flux",
								Source: "return _prepared\n    |> group(columns: [newColumn])",
								Start: ast.Position{
									Column: 3,
									Line:   116,
								},
							},
						},
					}},
				},
				Params: []*ast.Property{&ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 25,
								Line:   107,
							},
							File:   "geo.flux",
							Source: "tables=<-",
							Start: ast.Position{
								Column: 16,
								Line:   107,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 22,
									Line:   107,
								},
								File:   "geo.flux",
								Source: "tables",
								Start: ast.Position{
									Column: 16,
									Line:   107,
								},
							},
						},
						Name: "tables",
					},
					Value: &ast.PipeLiteral{BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 25,
								Line:   107,
							},
							File:   "geo.flux",
							Source: "<-",
							Start: ast.Position{
								Column: 23,
								Line:   107,
							},
						},
					}},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 36,
								Line:   107,
							},
							File:   "geo.flux",
							Source: "newColumn",
							Start: ast.Position{
								Column: 27,
								Line:   107,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 36,
									Line:   107,
								},
								File:   "geo.flux",
								Source: "newColumn",
								Start: ast.Position{
									Column: 27,
									Line:   107,
								},
							},
						},
						Name: "newColumn",
					},
					Value: nil,
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 43,
								Line:   107,
							},
							File:   "geo.flux",
							Source: "level",
							Start: ast.Position{
								Column: 38,
								Line:   107,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 43,
									Line:   107,
								},
								File:   "geo.flux",
								Source: "level",
								Start: ast.Position{
									Column: 38,
									Line:   107,
								},
							},
						},
						Name: "level",
					},
					Value: nil,
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 55,
								Line:   107,
							},
							File:   "geo.flux",
							Source: "ciLevel=-1",
							Start: ast.Position{
								Column: 45,
								Line:   107,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 52,
									Line:   107,
								},
								File:   "geo.flux",
								Source: "ciLevel",
								Start: ast.Position{
									Column: 45,
									Line:   107,
								},
							},
						},
						Name: "ciLevel",
					},
					Value: &ast.UnaryExpression{
						Argument: &ast.IntegerLiteral{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 55,
										Line:   107,
									},
									File:   "geo.flux",
									Source: "1",
									Start: ast.Position{
										Column: 54,
										Line:   107,
									},
								},
							},
							Value: int64(1),
						},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 55,
									Line:   107,
								},
								File:   "geo.flux",
								Source: "-1",
								Start: ast.Position{
									Column: 53,
									Line:   107,
								},
							},
						},
						Operator: 6,
					},
				}},
			},
		}, &ast.VariableAssignment{
			BaseNode: ast.BaseNode{
				Errors: nil,
				Loc: &ast.SourceLocation{
					End: ast.Position{
						Column: 30,
						Line:   124,
					},
					File:   "geo.flux",
					Source: "asTracks = (tables=<-, groupBy=[\"id\",\"tid\"], orderBy=[\"_time\"]) =>\n  tables\n    |> group(columns: groupBy)\n    |> sort(columns: orderBy)",
					Start: ast.Position{
						Column: 1,
						Line:   121,
					},
				},
			},
			ID: &ast.Identifier{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 9,
							Line:   121,
						},
						File:   "geo.flux",
						Source: "asTracks",
						Start: ast.Position{
							Column: 1,
							Line:   121,
						},
					},
				},
				Name: "asTracks",
			},
			Init: &ast.FunctionExpression{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 30,
							Line:   124,
						},
						File:   "geo.flux",
						Source: "(tables=<-, groupBy=[\"id\",\"tid\"], orderBy=[\"_time\"]) =>\n  tables\n    |> group(columns: groupBy)\n    |> sort(columns: orderBy)",
						Start: ast.Position{
							Column: 12,
							Line:   121,
						},
					},
				},
				Body: &ast.PipeExpression{
					Argument: &ast.PipeExpression{
						Argument: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 9,
										Line:   122,
									},
									File:   "geo.flux",
									Source: "tables",
									Start: ast.Position{
										Column: 3,
										Line:   122,
									},
								},
							},
							Name: "tables",
						},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 31,
									Line:   123,
								},
								File:   "geo.flux",
								Source: "tables\n    |> group(columns: groupBy)",
								Start: ast.Position{
									Column: 3,
									Line:   122,
								},
							},
						},
						Call: &ast.CallExpression{
							Arguments: []ast.Expression{&ast.ObjectExpression{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 30,
											Line:   123,
										},
										File:   "geo.flux",
										Source: "columns: groupBy",
										Start: ast.Position{
											Column: 14,
											Line:   123,
										},
									},
								},
								Properties: []*ast.Property{&ast.Property{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 30,
												Line:   123,
											},
											File:   "geo.flux",
											Source: "columns: groupBy",
											Start: ast.Position{
												Column: 14,
												Line:   123,
											},
										},
									},
									Key: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 21,
													Line:   123,
												},
												File:   "geo.flux",
												Source: "columns",
												Start: ast.Position{
													Column: 14,
													Line:   123,
												},
											},
										},
										Name: "columns",
									},
									Value: &ast.Identifier{
										BaseNode: ast.BaseNode{
											Errors: nil,
											Loc: &ast.SourceLocation{
												End: ast.Position{
													Column: 30,
													Line:   123,
												},
												File:   "geo.flux",
												Source: "groupBy",
												Start: ast.Position{
													Column: 23,
													Line:   123,
												},
											},
										},
										Name: "groupBy",
									},
								}},
								With: nil,
							}},
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 31,
										Line:   123,
									},
									File:   "geo.flux",
									Source: "group(columns: groupBy)",
									Start: ast.Position{
										Column: 8,
										Line:   123,
									},
								},
							},
							Callee: &ast.Identifier{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 13,
											Line:   123,
										},
										File:   "geo.flux",
										Source: "group",
										Start: ast.Position{
											Column: 8,
											Line:   123,
										},
									},
								},
								Name: "group",
							},
						},
					},
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 30,
								Line:   124,
							},
							File:   "geo.flux",
							Source: "tables\n    |> group(columns: groupBy)\n    |> sort(columns: orderBy)",
							Start: ast.Position{
								Column: 3,
								Line:   122,
							},
						},
					},
					Call: &ast.CallExpression{
						Arguments: []ast.Expression{&ast.ObjectExpression{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 29,
										Line:   124,
									},
									File:   "geo.flux",
									Source: "columns: orderBy",
									Start: ast.Position{
										Column: 13,
										Line:   124,
									},
								},
							},
							Properties: []*ast.Property{&ast.Property{
								BaseNode: ast.BaseNode{
									Errors: nil,
									Loc: &ast.SourceLocation{
										End: ast.Position{
											Column: 29,
											Line:   124,
										},
										File:   "geo.flux",
										Source: "columns: orderBy",
										Start: ast.Position{
											Column: 13,
											Line:   124,
										},
									},
								},
								Key: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 20,
												Line:   124,
											},
											File:   "geo.flux",
											Source: "columns",
											Start: ast.Position{
												Column: 13,
												Line:   124,
											},
										},
									},
									Name: "columns",
								},
								Value: &ast.Identifier{
									BaseNode: ast.BaseNode{
										Errors: nil,
										Loc: &ast.SourceLocation{
											End: ast.Position{
												Column: 29,
												Line:   124,
											},
											File:   "geo.flux",
											Source: "orderBy",
											Start: ast.Position{
												Column: 22,
												Line:   124,
											},
										},
									},
									Name: "orderBy",
								},
							}},
							With: nil,
						}},
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 30,
									Line:   124,
								},
								File:   "geo.flux",
								Source: "sort(columns: orderBy)",
								Start: ast.Position{
									Column: 8,
									Line:   124,
								},
							},
						},
						Callee: &ast.Identifier{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 12,
										Line:   124,
									},
									File:   "geo.flux",
									Source: "sort",
									Start: ast.Position{
										Column: 8,
										Line:   124,
									},
								},
							},
							Name: "sort",
						},
					},
				},
				Params: []*ast.Property{&ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 22,
								Line:   121,
							},
							File:   "geo.flux",
							Source: "tables=<-",
							Start: ast.Position{
								Column: 13,
								Line:   121,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 19,
									Line:   121,
								},
								File:   "geo.flux",
								Source: "tables",
								Start: ast.Position{
									Column: 13,
									Line:   121,
								},
							},
						},
						Name: "tables",
					},
					Value: &ast.PipeLiteral{BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 22,
								Line:   121,
							},
							File:   "geo.flux",
							Source: "<-",
							Start: ast.Position{
								Column: 20,
								Line:   121,
							},
						},
					}},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 44,
								Line:   121,
							},
							File:   "geo.flux",
							Source: "groupBy=[\"id\",\"tid\"]",
							Start: ast.Position{
								Column: 24,
								Line:   121,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 31,
									Line:   121,
								},
								File:   "geo.flux",
								Source: "groupBy",
								Start: ast.Position{
									Column: 24,
									Line:   121,
								},
							},
						},
						Name: "groupBy",
					},
					Value: &ast.ArrayExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 44,
									Line:   121,
								},
								File:   "geo.flux",
								Source: "[\"id\",\"tid\"]",
								Start: ast.Position{
									Column: 32,
									Line:   121,
								},
							},
						},
						Elements: []ast.Expression{&ast.StringLiteral{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 37,
										Line:   121,
									},
									File:   "geo.flux",
									Source: "\"id\"",
									Start: ast.Position{
										Column: 33,
										Line:   121,
									},
								},
							},
							Value: "id",
						}, &ast.StringLiteral{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 43,
										Line:   121,
									},
									File:   "geo.flux",
									Source: "\"tid\"",
									Start: ast.Position{
										Column: 38,
										Line:   121,
									},
								},
							},
							Value: "tid",
						}},
					},
				}, &ast.Property{
					BaseNode: ast.BaseNode{
						Errors: nil,
						Loc: &ast.SourceLocation{
							End: ast.Position{
								Column: 63,
								Line:   121,
							},
							File:   "geo.flux",
							Source: "orderBy=[\"_time\"]",
							Start: ast.Position{
								Column: 46,
								Line:   121,
							},
						},
					},
					Key: &ast.Identifier{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 53,
									Line:   121,
								},
								File:   "geo.flux",
								Source: "orderBy",
								Start: ast.Position{
									Column: 46,
									Line:   121,
								},
							},
						},
						Name: "orderBy",
					},
					Value: &ast.ArrayExpression{
						BaseNode: ast.BaseNode{
							Errors: nil,
							Loc: &ast.SourceLocation{
								End: ast.Position{
									Column: 63,
									Line:   121,
								},
								File:   "geo.flux",
								Source: "[\"_time\"]",
								Start: ast.Position{
									Column: 54,
									Line:   121,
								},
							},
						},
						Elements: []ast.Expression{&ast.StringLiteral{
							BaseNode: ast.BaseNode{
								Errors: nil,
								Loc: &ast.SourceLocation{
									End: ast.Position{
										Column: 62,
										Line:   121,
									},
									File:   "geo.flux",
									Source: "\"_time\"",
									Start: ast.Position{
										Column: 55,
										Line:   121,
									},
								},
							},
							Value: "_time",
						}},
					},
				}},
			},
		}},
		Imports:  nil,
		Metadata: "parser-type=go",
		Name:     "geo.flux",
		Package: &ast.PackageClause{
			BaseNode: ast.BaseNode{
				Errors: nil,
				Loc: &ast.SourceLocation{
					End: ast.Position{
						Column: 12,
						Line:   2,
					},
					File:   "geo.flux",
					Source: "package geo",
					Start: ast.Position{
						Column: 1,
						Line:   2,
					},
				},
			},
			Name: &ast.Identifier{
				BaseNode: ast.BaseNode{
					Errors: nil,
					Loc: &ast.SourceLocation{
						End: ast.Position{
							Column: 12,
							Line:   2,
						},
						File:   "geo.flux",
						Source: "geo",
						Start: ast.Position{
							Column: 9,
							Line:   2,
						},
					},
				},
				Name: "geo",
			},
		},
	}},
	Package: "geo",
	Path:    "experimental/geo",
}
