...
func TestClickhouseParsing(t *testing.T) {
	// here we want to build a mocked representation of what's in our ClickHouse db, and then run our RowReader over it, then verify that the results
	// are as expected
	testCases := []struct {
		name       string
		columnName string
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			TestReader, err := NewClickhouseRowReader(tc.data)
			if !cmp.Equal(nil, err) {
				t.Fatalf("unexpected result -want/+got\n\n%s\n\n", cmp.Diff(nil, err))
			}
			i := 0
