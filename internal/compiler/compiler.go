...
func Compile(f *semantic.FunctionExpression, in semantic.MonoType) (Func, error) {
...
type additionEvaluator struct {
	t     semantic.MonoType
	left  FloatEvaluator
	right FloatEvaluator
}

func (e *additionEvaluator) Type() semantic.MonoType {
	return e.t
}

func (e *additionEvaluator) Eval(ctx context.Context, scope Scope) (Value, error) {
	v, err := e.EvalFloat(ctx, scope)
	if err != nil {
		return Value{}, err
	}
...
func (e *additionEvaluator) EvalFloat(ctx context.Context, scope Scope) (float64, error) {
	lv, err := e.left.EvalFloat(ctx, scope)
	if err != nil {
		return 0, err
	}
	rv, err := e.right.EvalFloat(ctx, scope)
	if err != nil {
		return 0, err
	}
	return lv + rv, nil
...
type FloatEvaluator interface {
	Evaluator
	EvalFloat(ctx context.Context, scope Scope) (float64, error)
}

type BoolEvaluator interface {
	Evaluator
	EvalBool(ctx context.Context, scope Scope) (bool, error)
}

type floatComparisonEvaluator struct {
	t     semantic.MonoType
	left  FloatEvaluator
	right FloatEvaluator
}

func (e *floatComparisonEvaluator) Type() semantic.MonoType {
	return e.t
}

func (e *floatComparisonEvaluator) Eval(ctx context.Context, scope Scope) (Value, error) {
	v, err := e.EvalBool(ctx, scope)
	if err != nil {
		return Value{}, err
	}
...
func (e *floatComparisonEvaluator) EvalBool(ctx context.Context, scope Scope) (bool, error) {
	lv, err := e.left.EvalFloat(ctx, scope)
	if err != nil {
		return false, err
	}
...
func compile(n semantic.Node, subst map[uint64]semantic.MonoType, scope Scope) (Evaluator, error) {
	...
	case *semantic.NativeVariableAssignment:
		...
		return &declarationEvaluator{
			...
			id:   scope.Declare(n.Identifier.Name),
			init: node,
		}, nil
	...
	case *semantic.MemberExpression:
		...
		t := apply(subst, nil, n.TypeOf())
		index, err := findPropertyIndex(n.Property, t)
		if err != nil {
			return nil, err
		}
	...
	case *semantic.BinaryExpression:
		...
		t := apply(subst, nil, n.TypeOf())
		if n.Operator == ast.AdditionOperator {
			return &additionEvaluator{
				t:     t,
				left:  l.(FloatEvaluator),
				right: r.(FloatEvaluator),
			}, nil
		} else if n.Operator == ast.GreaterThanOperator {
			return &floatComparisonEvaluator{
				t:     t,
				left:  l.(FloatEvaluator),
				right: r.(FloatEvaluator),
			}, nil
