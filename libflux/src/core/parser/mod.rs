...
impl Parser {
    ...
    fn parse_builtin_statement(&mut self) -> Statement {
        // "builtin" identifer ":" TypeExpression
        let t = self.expect(TOK_BUILTIN); // builtin
        let id = self.parse_identifier(); // identifier
        self.expect(TOK_COLON); // :
        let type_expression = self.parse_type_expression(); // TypeExpression
        Statement::Builtin(Box::new(BuiltinStmt {
            base: self.base_node_from_other_end(&t, &type_expression.base),
            id,
