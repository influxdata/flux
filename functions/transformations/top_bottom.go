...
_sortLimit = (n, desc, columns=["_value"], table=<-) =>
    table
        |> sort(columns:columns, desc:desc)
        |> limit(n:n)

// top sorts a table by columns and keeps only the top n records.
top = (n, columns=["_value"], table=<-) => _sortLimit(table:table, n:n, columns:columns, desc:true)

// top sorts a table by columns and keeps only the bottom n records.
bottom = (n, columns=["_value"], table=<-) => _sortLimit(table:table, n:n, columns:columns, desc:false)
...
_highestOrLowest = (n, _sortLimit, reducer, columns=["_value"], by=[], table=<-) =>
    table
        |> group(by:by)
        |> reducer()
        |> group(none:true)
...
highestMax = (n, columns=["_value"], by=[], table=<-) =>
    _highestOrLowest(
        table:table,
        n:n,
        columns:columns,
...
lowestCurrent = (n, columns=["_value"], by=[], table=<-) =>
    _highestOrLowest(
        table: table,
        n:n,
        columns:columns,
        by:by,
        reducer: (table=<-) => table |> last(column:columns[0]),
        _sortLimit: bottom,
    )
`
