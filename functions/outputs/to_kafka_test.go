...
func TestToKafka_Process(t *testing.T) {
	data := &kafkaMock{}
	outputs.DefaultKafkaWriterFactory = func(_ kafka.WriterConfig) outputs.KafkaWriter {
		return data
	}
	...
	testCases := []struct {
		name string
		spec *outputs.ToKafkaProcedureSpec
		data []flux.Table
		want wanted
	}{
		{
		...
		t.Run(tc.name, func(t *testing.T) {
			executetest.ProcessTestHelper(
				...
				func(d execute.Dataset, c execute.TableBuilderCache) execute.Transformation {
					return outputs.NewToKafkaTransformation(d, c, tc.spec)
				},
			)
